...
function createMockVideo({ currentTime = 0, duration = 3600, paused = true, volume = 1, muted = false, playbackRate = 1, } = {}) {
...
function Wrapper({ video, alternatives = [], audioTracks = [], subtitleTracks = [], currentSubtitleTrack = null, markers = [], episodeLabel = null, nextEpisode = null, onNextEpisode = vi.fn(), onClose = vi.fn(), onVariantSwitch = vi.fn(), onAudioTrack = vi.fn(), onSubtitleTrack = vi.fn(), deliveryMode = null, containerExtension = null, }) {
    const videoRef = useRef(video);
    return (_jsx(PlayerControls, { videoRef: videoRef, alternatives: alternatives, onVariantSwitch: onVariantSwitch, onClose: onClose, audioTracks: audioTracks, subtitleTracks: subtitleTracks, currentSubtitleTrack: currentSubtitleTrack, onAudioTrack: onAudioTrack, onSubtitleTrack: onSubtitleTrack, markers: markers, episodeLabel: episodeLabel, nextEpisode: nextEpisode, onNextEpisode: onNextEpisode, deliveryMode: deliveryMode, containerExtension: containerExtension }));
}
describe('PlayerControls', () => {
    ...
    it('renders play button when paused', () => {
        render(_jsx(Wrapper, { video: video }));
    ...
    it('audio popover opens and lists tracks', () => {
        ...
        render(_jsx(Wrapper, { video: video, audioTracks: tracks }));
    ...
    it('subtitle popover always includes Désactivés option', () => {
        const tracks = [{ id: 0, label: 'Français', lang: 'fr' }];
        render(_jsx(Wrapper, { video: video, subtitleTracks: tracks }));
    ...
    it('onSubtitleTrack called with null when Désactivés clicked', () => {
        ...
        const tracks = [{ id: 0, label: 'Français', lang: 'fr' }];
        render(_jsx(Wrapper, { video: video, subtitleTracks: tracks, onSubtitleTrack: onSubtitleTrack }));
        fireEvent.click(screen.getByLabelText('Sous-titres'));
    ...
    it('CC button hidden for DIRECT MP4 with no subtitle tracks', () => {
        render(_jsx(Wrapper, { video: video, deliveryMode: "DIRECT", containerExtension: "mp4", subtitleTracks: [] }));
    ...
    it('CC button shown for DIRECT MKV with no subtitle tracks', () => {
        render(_jsx(Wrapper, { video: video, deliveryMode: "DIRECT", containerExtension: "mkv", subtitleTracks: [] }));
    ...
    it('CC button shown when subtitle tracks exist regardless of container', () => {
        const tracks = [{ id: 0, label: 'Français', lang: 'fr' }];
        render(_jsx(Wrapper, { video: video, deliveryMode: "DIRECT", containerExtension: "mp4", subtitleTracks: tracks }));
        expect(screen.getByLabelText('Sous-titres')).toBeInTheDocument();
    });
    ...
    it('shows marker button within marker range', () => {
        const markers = [{ type: 'intro', startSeconds: 0, endSeconds: 60 }];
        const v = createMockVideo({ currentTime: 30, duration: 3600 });
        render(_jsx(Wrapper, { video: v, markers: markers }));
    ...
    it('shows episode label in top bar when provided', () => {
        render(_jsx(Wrapper, { video: video, episodeLabel: "E01 \u00B7 Titre" }));
        expect(screen.getByText('E01 · Titre')).toBeInTheDocument();
    });
    it('shows next episode button when nextEpisode provided', () => {
        const ep = { id: 'ep-2', title: 'Ep 2', episodeNumber: 2, synopsis: null, durationMinutes: null, airDate: null, availabilityCount: 1, availabilityStatus: 'AVAILABLE', selectedVariantId: null, variants: [], watchState: null, posterUrl: null };
        render(_jsx(Wrapper, { video: video, nextEpisode: ep }));
    ...
    it('calls onNextEpisode when next episode button clicked', () => {
        ...
        const ep = { id: 'ep-2', title: 'Ep 2', episodeNumber: 2, synopsis: null, durationMinutes: null, airDate: null, availabilityCount: 1, availabilityStatus: 'AVAILABLE', selectedVariantId: null, variants: [], watchState: null, posterUrl: null };
        render(_jsx(Wrapper, { video: video, nextEpisode: ep, onNextEpisode: onNextEpisode }));
...
describe('PlayerControls controls visibility', () => {
    ...
    it('pointer move on wrapper restores controls after auto-hide', () => {
        const { container } = render(_jsx(Wrapper, { video: video }));
        ...
        act(() => { fireEvent.pointerMove(container.firstChild); });
    ...
    it('source change resets scrubbing state so hide timer works on restart', () => {
        const { unmount } = render(_jsx(Wrapper, { video: video }));
        act(() => { video.dispatchEvent(new Event('play')); });
...
describe('PlayerControls keyboard shortcuts', () => {
    ...
    it('keyboard shortcuts do not fire when typing in an input', () => {
        render(_jsxs(_Fragment, { children: [_jsx("input", { "data-testid": "txt" }), _jsx(Wrapper, { video: video })] }));
...
//# sourceMappingURL=PlayerControls.test.js.map