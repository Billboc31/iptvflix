...
export default function SeasonSelector({ seriesId, seasons, profileId, devices, progressByEpisodeId }) {
    ...
    useEffect(() => {
        if (seasons.length === 0)
            return;
        if (episodeCache.has(selectedSeason) && (episodeCache.get(selectedSeason)?.length ?? 0) > 0)
            return;
        let cancelled = false;
        setLoading(true);
