...
describe('DevicePickerModal', () => {
  it('renders device list with online and offline indicators', () => {
    render(
      <DevicePickerModal
        {...baseProps}
        devices={[MOCK_DEVICE_ONLINE, MOCK_DEVICE_OFFLINE]}
      />,
    )
  ...
  it('fast-path skips modal for single device and calls onFastPath', async () => {
    ...
    render(
      <DevicePickerModal
        {...baseProps}
        onClose={onClose}
        devices={[MOCK_DEVICE_ONLINE]}
        onFastPath={onFastPath}
      />,
    )
  ...
  it('shows resume toggle when progressMs > 0 and sends correct startPositionMs', async () => {
    ...
    server.use(
      http.post('/api/devices/:id/commands', async ({ request }) => {
  ...
  it('shows error message and does not auto-close on API failure', async () => {
    ...
    const onClose = vi.fn()
    server.use(
      http.post('/api/devices/:id/commands', () => new HttpResponse(null, { status: 500 })),
    )
