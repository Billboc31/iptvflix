...
function renderCard(item, onDismiss = vi.fn().mockResolvedValue(undefined), initialPath = '/', dismissError) {
    return render(_jsx(MemoryRouter, { initialEntries: [initialPath], children: _jsx(Routes, { children: _jsx(Route, { path: "*", element: _jsx(ContinueWatchingCard, { item: item, onDismiss: onDismiss, dismissError: dismissError }) }) }) }));
}
describe('ContinueWatchingCard — MOVIE', () => {
    ...
    it('play button navigates with source=continue_watching', async () => {
        ...
        render(_jsx(MemoryRouter, { initialEntries: ['/'], children: _jsxs(Routes, { children: [_jsx(Route, { path: "/", element: _jsx(ContinueWatchingCard, { item: MOCK_CONTINUE_WATCHING, onDismiss: vi.fn().mockResolvedValue(undefined) }) }), _jsx(Route, { path: "/player/movie/:id", element: _jsx("div", { "data-testid": "player-page", children: "PlayerPage" }) })] }) }));
...
describe('ContinueWatchingCard — info button navigation', () => {
    it('MOVIE: clicking ⓘ navigates to /movies/:id', async () => {
        const user = userEvent.setup();
        render(_jsx(MemoryRouter, { initialEntries: ['/'], children: _jsxs(Routes, { children: [_jsx(Route, { path: "/", element: _jsx(ContinueWatchingCard, { item: MOCK_CONTINUE_WATCHING, onDismiss: vi.fn().mockResolvedValue(undefined) }) }), _jsx(Route, { path: "/movies/:id", element: _jsx("div", { "data-testid": "movie-detail", children: "MovieDetail" }) })] }) }));
    ...
    it('EPISODE: clicking ⓘ navigates to /series/:seriesId', async () => {
        const user = userEvent.setup();
        render(_jsx(MemoryRouter, { initialEntries: ['/'], children: _jsxs(Routes, { children: [_jsx(Route, { path: "/", element: _jsx(ContinueWatchingCard, { item: MOCK_CONTINUE_WATCHING_EPISODE, onDismiss: vi.fn().mockResolvedValue(undefined) }) }), _jsx(Route, { path: "/series/:id", element: _jsx("div", { "data-testid": "series-detail", children: "SeriesDetail" }) })] }) }));
        await user.click(screen.getByRole('button', { name: 'Voir les détails' }));
...
describe('ContinueWatchingCard — direct resume (no resume dialog)', () => {
    it('DELETE /continue-watching is called with correct params on dismiss via API handler', async () => {
        const user = userEvent.setup();
        server.use(http.delete('/api/continue-watching/:mediaType/:mediaId', () => {
...
//# sourceMappingURL=ContinueWatchingCard.test.js.map