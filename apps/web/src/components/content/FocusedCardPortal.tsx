...
export default function FocusedCardPortal({
  cardRect,
  title,
  posterUrl,
  trailerKey,
  ...
  return createPortal(
    <div
      data-testid="focused-card-portal"
      aria-hidden="true"
      style={{
        position: 'fixed',
        left,
      ...
      className="rounded-xl overflow-hidden shadow-2xl bg-[#1a1a24]"
    >
      {/* Backdrop image */}
      {posterUrl && (
        <img
      ...
      {/* Preview player — muted, lazy-mounted by PreviewContext */}
      ...
      <div className="absolute inset-x-0 bottom-0 p-3 flex flex-col gap-2">
        <p className="text-white text-sm font-semibold line-clamp-2 leading-tight drop-shadow">
          {title}
        </p>
        <button
          onClick={onDetailsClick}
          className="w-full px-3 py-1.5 bg-white/20 hover:bg-white/30 text-white text-xs font-semibold rounded-full backdrop-blur-sm transition-colors"
          tabIndex={-1}
        >
          Détails
        </button>
