...
describe('HeroSection', () => {
    ...
    it('renders the title', () => {
        render(_jsx(HeroSection, { title: "Inception" }));
        expect(screen.getByText('Inception')).toBeInTheDocument();
    });
    it('does not mount preview player without trailerKey', async () => {
        render(_jsx(HeroSection, { title: "Movie", mediaId: "movie-1" }));
    ...
    it('does not mount preview player without mediaId', async () => {
        render(_jsx(HeroSection, { title: "Movie", trailerKey: "abc123" }));
        await act(() => vi.advanceTimersByTime(3000));
        expect(screen.queryByTestId('preview-iframe')).not.toBeInTheDocument();
    });
    it('mounts preview player after 2s delay when trailerKey and mediaId are provided', async () => {
        ...
        render(_jsx(HeroSection, { title: "Movie", mediaId: "movie-1", trailerKey: "abc123" }));
    ...
    it('cleans up timer and deactivates on unmount', async () => {
        const { unmount } = render(_jsx(HeroSection, { title: "Movie", mediaId: "movie-1", trailerKey: "abc123" }));
        unmount();
        await act(() => vi.advanceTimersByTime(3000));
        expect(screen.queryByTestId('preview-iframe')).not.toBeInTheDocument();
    });
    ...
    it('renders a Play button when availabilityStatus is AVAILABLE and onPlay is provided', () => {
        const onPlay = vi.fn();
        render(_jsx(HeroSection, { title: "Movie", availabilityStatus: "AVAILABLE", onPlay: onPlay }));
    ...
    it('does not render Play button when availabilityStatus is UNAVAILABLE', () => {
        render(_jsx(HeroSection, { title: "Movie", availabilityStatus: "UNAVAILABLE", onPlay: vi.fn() }));
    ...
    it('does not render Play button when onPlay is absent even if availabilityStatus is AVAILABLE', () => {
        render(_jsx(HeroSection, { title: "Movie", availabilityStatus: "AVAILABLE" }));
        expect(screen.queryByRole('button', { name: 'Lire' })).not.toBeInTheDocument();
    });
...
//# sourceMappingURL=HeroSection.test.js.map