...
describe('SeriesDetailPage', () => {
    ...
    it('expanding a season fetches and displays its episode list', async () => {
        ...
        const seasonBtn = screen.getAllByRole('button').find((btn) => btn.textContent?.includes('Saison 1'));
    ...
    it('caches episodes so a second expand does not re-fetch', async () => {
        ...
        const [seasonBtn] = screen.getAllByRole('button').filter((btn) => btn.textContent?.includes('Saison 1'));
        await user.click(seasonBtn);
