...
function renderPlayerPage(mediaType = 'movie', mediaId = 'movie-1', searchParams = '') {
    return render(_jsx(MemoryRouter, { initialEntries: [`/player/${mediaType}/${mediaId}${searchParams}`], children: _jsx(Routes, { children: _jsx(Route, { path: "/player/:mediaType/:mediaId", element: _jsx(PlayerPage, {}) }) }) }));
}
describe('PlayerPage', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        server.use(http.post('/api/playback/resolve/:mediaType/:mediaId', () => HttpResponse.json(BASE_PLAYBACK)), http.get('/api/progress/:mediaType/:mediaId', () => HttpResponse.json([])));
    });
    afterEach(() => {
    ...
    it('regression: calls HLS.js loadSource with the gateway URL from the playback session', async () => {
        ...
        const url = mockLoadSource.mock.calls[0]?.[0] ?? '';
    ...
    it('regression: calls HLS.js attachMedia with the video element', async () => {
        ...
        const arg = mockAttachMedia.mock.calls[0]?.[0];
    ...
    it('does not show resume dialog when opened from the continue-watching row', async () => {
        server.use(http.post('/api/playback/resolve/:mediaType/:mediaId', () => HttpResponse.json({ ...BASE_PLAYBACK, startPositionSeconds: 600 })));
    ...
    it('"Reprendre" button dismisses the resume dialog', async () => {
        ...
        const resumeBtn = screen.getAllByRole('button').find((b) => b.textContent?.includes('Reprendre à'));
    ...
    it('shows resume dialog for episode with startPositionSeconds > 30', async () => {
        server.use(http.post('/api/playback/resolve/:mediaType/:mediaId', () => HttpResponse.json({ ...BASE_PLAYBACK, startPositionSeconds: 120 })));
    ...
    it('does not show resume dialog when startPositionSeconds is below threshold (20 s)', async () => {
        server.use(http.post('/api/playback/resolve/:mediaType/:mediaId', () => HttpResponse.json({ ...BASE_PLAYBACK, startPositionSeconds: 20 })));
        renderPlayerPage();
        await waitFor(() => expect(mockLoadSource).toHaveBeenCalled());
        const video = document.querySelector('video');
        Object.defineProperty(video, 'duration', { get: () => 7200, configurable: true });
    ...
    it('does not show resume dialog when startPositionSeconds is near the end (within 60 s of duration)', async () => {
        server.use(http.post('/api/playback/resolve/:mediaType/:mediaId', () => HttpResponse.json({ ...BASE_PLAYBACK, startPositionSeconds: 3550 })));
        renderPlayerPage();
        await waitFor(() => expect(mockLoadSource).toHaveBeenCalled());
        const video = document.querySelector('video');
...
//# sourceMappingURL=PlayerPage.test.js.map