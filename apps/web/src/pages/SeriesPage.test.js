...
function renderPage() {
    return render(_jsx(MemoryRouter, { children: _jsx(SeriesPage, {}) }));
}
describe('SeriesPage', () => {
    ...
    it('shows no hero when API returns no series', async () => {
        ...
        server.use(http.get('/api/series', () => HttpResponse.json({ items: [], total: 0, page: 1, pageSize: 20 })));
        renderPage();
        await waitFor(() => {
            expect(screen.queryByRole('button', { name: 'Lire' })).not.toBeInTheDocument();
...
//# sourceMappingURL=SeriesPage.test.js.map