...
vi.mock('../components/content/ContinueWatchingRow.js', () => ({
  default: () => <div>Continuer à regarder</div>,
}))
...
const mockShelfRowImpl = vi.fn(({ shelf }: { shelf: ShelfResponse }) => {
  if (!shelf.items?.length) return null
  return <div data-testid="shelf-row" data-shelf-id={shelf.id}>{shelf.title}</div>
})
vi.mock('../components/content/ShelfRow.js', () => ({
  default: (props: { shelf: ShelfResponse }) => mockShelfRowImpl(props),
}))

// useInfiniteHome is the main hook under test.
const mockUseInfiniteHome = vi.fn()
vi.mock('../hooks/useHome.js', () => ({
  useInfiniteHome: (...args: unknown[]) => mockUseInfiniteHome(...args),
}))
...
function makeShelf(id: string, title: string, items: ShelfResponse['items'] = []): ShelfResponse {
  return { id, title, type: 'GENERATED', layoutHint: 'ROW', items }
}

function makeItem(mediaId: string): ShelfResponse['items'][number] {
  return { mediaType: 'MOVIE', mediaId, title: 'A Film', posterUrl: null }
}

const DEFAULT_HOME_STATE = {
  allShelves: [] as ShelfResponse[],
  sessionId: 'session-1',
  nextCursor: null,
  isLoading: false,
...
beforeEach(() => {
  vi.clearAllMocks()
  mockShelfRowImpl.mockImplementation(({ shelf }: { shelf: ShelfResponse }) => {
    if (!shelf.items?.length) return null
    return <div data-testid="shelf-row" data-shelf-id={shelf.id}>{shelf.title}</div>
  })
...
describe('HomePage — ShelfErrorBoundary', () => {
  it('error thrown by one ShelfRow does not unmount other ShelfRow instances', async () => {
    ...
    mockShelfRowImpl.mockImplementation(({ shelf }: { shelf: ShelfResponse }) => {
      if (shelf.id === 'shelf-crash') throw new Error('render explosion')
      if (!shelf.items?.length) return null
      return <div data-testid="shelf-row" data-shelf-id={shelf.id}>{shelf.title}</div>
    })
