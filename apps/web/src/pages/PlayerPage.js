...
export default function PlayerPage() {
    ...
    const handleStableDuration = useCallback((seconds) => {
    ...
    const { flushProgress } = useProgressSync(videoRef, progressMediaType, mediaId, status === 'ready' && progressSyncReady, stableDurationSeconds, sessionIdRef.current, progressFloorSeconds, deliveryMode != null && deliveryMode !== 'DIRECT' && startPositionSeconds > 30
    ...
    useEffect(() => {
        return () => {
            const video = videoRef.current;
            if (!video || !mediaId || !hasPlayedRef.current || video.ended)
                return;
    ...
    useEffect(() => {
        if (!videoRef.current || !gatewayUrl || !deliveryMode)
            return;
        const video = videoRef.current;
                ...
                try {
                    ...
                    if (Hls.isSupported()) {
                        hlsInstance = new Hls({
                            ...
                            startPosition: deliveryMode === 'DIRECT' && startPositionRef.current > RESUME_THRESHOLD_START_S
                        ...
                        hlsInstance.on(Hls.Events.ERROR, (_event, data) => {
                            ...
                            if (status)
                                httpStatusRef.current = status;
                            setVideoError(videoErrorMessage(video, status));
                        });
                        // Audio track management
                        hlsInstance.on(Hls.Events.AUDIO_TRACKS_UPDATED, () => {
                            if (cancelled || !hlsInstance)
                                return;
                            setAudioTracks(hlsInstance.audioTracks.map((t) => ({
                                id: t.id,
                        ...
                        hlsInstance.on(Hls.Events.SUBTITLE_TRACKS_UPDATED, () => {
                            if (cancelled || !hlsInstance)
                                return;
                            setSubtitleTracks(hlsInstance.subtitleTracks.map((t) => ({
                                id: t.id,
                        ...
                        hlsInstance.on(Hls.Events.SUBTITLE_TRACK_SWITCH, (_e, data) => {
                            if (!cancelled)
                                setCurrentSubtitleTrack(data.id < 0 ? null : data.id);
                        });
    ...
    useEffect(() => {
        ...
        if (!video || status !== 'ready')
            return;
        function decide() {
            if (!video || resumeHandledRef.current)
                return;
            const start = startPositionRef.current;
