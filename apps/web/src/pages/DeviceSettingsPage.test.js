...
function renderPage() {
    return render(_jsx(MemoryRouter, { children: _jsx(DeviceSettingsPage, {}) }));
}
describe('DeviceSettingsPage', () => {
    ...
    it('pairing approval: valid code shows new device', async () => {
        ...
        server.use(http.get('/api/devices', () => HttpResponse.json([])), http.get('/api/pairing/codes/:code', () => HttpResponse.json(MOCK_PAIRING_CODE_DETAIL)), http.post('/api/pairing/codes/:code/approve', () => HttpResponse.json(MOCK_DEVICE_ONLINE, { status: 201 })));
        renderPage();
        await waitFor(() => {
            expect(screen.getByText(/aucun appareil jumelé/i)).toBeInTheDocument();
        });
    ...
    it('pairing approval: expired code shows specific error', async () => {
        const user = userEvent.setup();
        server.use(http.get('/api/pairing/codes/:code', () => HttpResponse.json({ ...MOCK_PAIRING_CODE_DETAIL, status: 'expired' })));
        renderPage();
    ...
    it('pairing approval: unknown code shows error', async () => {
        const user = userEvent.setup();
        server.use(http.get('/api/pairing/codes/:code', () => new HttpResponse(null, { status: 404 })));
        renderPage();
        await waitFor(() => expect(screen.getByRole('button', { name: /jumeler/i })).toBeInTheDocument());
    ...
    it('rename device: inline edit updates device name', async () => {
        const user = userEvent.setup();
        server.use(http.patch('/api/devices/:id', () => HttpResponse.json({ ...MOCK_DEVICE_ONLINE, name: 'Living Room' })));
        renderPage();
    ...
    it('shows online/offline status', async () => {
        server.use(http.get('/api/devices', () => HttpResponse.json([MOCK_DEVICE_ONLINE, MOCK_DEVICE_OFFLINE])));
        renderPage();
        await waitFor(() => {
            expect(screen.getByText('Salon TV')).toBeInTheDocument();
        });
...
//# sourceMappingURL=DeviceSettingsPage.test.js.map