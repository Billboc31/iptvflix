...
function renderPage(id = 'movie-1') {
    return render(_jsx(ProfileProvider, { children: _jsx(MemoryRouter, { initialEntries: [`/movies/${id}`], children: _jsx(Routes, { children: _jsx(Route, { path: "/movies/:id", element: _jsx(MovieDetailPage, {}) }) }) }) }));
}
describe('MovieDetailPage', () => {
    ...
    it('shows "Données manquantes" badge when enrichmentStatus is unmatched', async () => {
        server.use(http.get('/api/movies/:id', () => HttpResponse.json(MOCK_UNMATCHED_MOVIE)));
        renderPage('movie-2');
        await waitFor(() => {
    ...
    it('shows "Données partielles" badge when enrichmentStatus is partial', async () => {
        server.use(http.get('/api/movies/:id', () => HttpResponse.json({ ...MOCK_MOVIE, enrichmentStatus: 'partial' })));
        renderPage();
        await waitFor(() => {
    ...
    it('shows not-found message when API returns 404', async () => {
        server.use(http.get('/api/movies/:id', () => new HttpResponse(null, { status: 404 })));
    ...
    it('shows error state on network failure', async () => {
        server.use(http.get('/api/movies/:id', () => new HttpResponse(null, { status: 500 })));
        renderPage();
        await waitFor(() => {
    ...
    it('does not show bande-annonce button when trailerKey is null', async () => {
        server.use(http.get('/api/movies/:id', () => HttpResponse.json(MOCK_MOVIE_NO_TRAILER)));
        renderPage('movie-3');
        await waitFor(() => {
...
//# sourceMappingURL=MovieDetailPage.test.js.map