...
function renderManage() {
    return render(_jsx(MemoryRouter, { initialEntries: ['/profiles'], children: _jsx(ProfileProvider, { children: _jsx(ProfileManagePage, {}) }) }));
}
describe('ProfileManagePage', () => {
    beforeEach(() => {
        localStorage.clear();
        localStorage.setItem('iptvflix_last_profile_id', MOCK_PROFILE_A.id);
        server.use(http.post('/api/profiles/:id/select', () => HttpResponse.json(MOCK_SELECT_PROFILE_RESPONSE)));
    });
    ...
    it('delete button is disabled when only one profile exists', async () => {
        server.use(http.get('/api/profiles', () => HttpResponse.json([MOCK_PROFILE_A])));
        renderManage();
        await waitFor(() => expect(screen.queryByText(MOCK_PROFILE_A.name)).toBeInTheDocument());
    ...
    it('shows 409 error message when delete is blocked by API', async () => {
        server.use(http.delete('/api/profiles/:id', () => HttpResponse.json({ error: 'Cannot delete the last profile' }, { status: 409 })));
        ...
        await waitFor(() => expect(screen.queryByText(/cannot delete the last profile/i)).toBeInTheDocument());
    });
    ...
    it('Ajouter button is disabled and limit message shown when at 5 profiles', async () => {
        ...
        server.use(http.get('/api/profiles', () => HttpResponse.json(profiles)));
        renderManage();
...
//# sourceMappingURL=ProfileManage.test.js.map