...
describe('useDevices', () => {
  it('loads devices on mount, filtering out revoked ones', async () => {
    ...
    server.use(
      http.get('/api/devices', () => HttpResponse.json([MOCK_DEVICE_ONLINE, revoked])),
    )
  ...
  it('approve adds the new device to the list', async () => {
    ...
    server.use(
      ...
      http.post('/api/pairing/codes/:code/approve', () => HttpResponse.json(newDevice, { status: 201 })),
    )
  ...
  it('rename updates the device name in the list', async () => {
    ...
    server.use(
      http.patch('/api/devices/:id', () => HttpResponse.json(renamed)),
    )
