...
export function useNeverStop(videoRef: RefObject<HTMLVideoElement | null>, segments: EpisodeSegmentItem[], preferences: Partial<ProfilePreferences>, enabled: boolean, timelineOffsetSeconds: number, mediaKey: string, onNext: () => boolean, totalDurationSeconds?: number | null) {
  const [active, setActive] = useState<EpisodeSegmentItem>()
  const skipped = useRef(new Set<string>())
  const next = useRef(onNext)
  next.current = onNext
  useEffect(() => { skipped.current.clear(); setActive(undefined) }, [mediaKey])
  const skip = (segment: EpisodeSegmentItem) => {
  ...
  useEffect(() => {
    const video = videoRef.current
    if (!video || !enabled) { setActive(undefined); return }
    const tick = () => {
      const segment = activeSegment(segments, (video.currentTime + timelineOffsetSeconds) * 1000)
      setActive(segment)
      if (!segment || video.paused || video.seeking || video.ended || !shouldAutoSkip(segment, preferences)) return
      const key = `${segment.type}:${segment.startMs}:${segment.endMs}`
      if (preferences.neverStopMode && coversEnd(segment, segments, (totalDurationSeconds ?? NaN) * 1000) && next.current()) return
      if (!skipped.current.has(key) && skip(segment)) { skipped.current.add(key); setActive(undefined) }
    }
    const ended = () => { if (preferences.neverStopMode || preferences.autoplayNextEpisode) next.current() }
