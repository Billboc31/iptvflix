      ...
      <div className="flex flex-wrap gap-8 justify-center max-w-2xl">
        {profiles.map((profile) => {
          ...
          return (
            ...
            >
              <div className="relative w-24 h-24 rounded-full bg-[#1a1a24] border-2 border-white/10 group-hover:border-[#f97316] transition-colors flex items-center justify-center">
                <span className="text-3xl font-bold text-white">
                  {profile.name.charAt(0).toUpperCase()}
                </span>
                {isSelecting && (
                  <div className="absolute inset-0 flex items-center justify-center rounded-full bg-black/50">
                    <span className="w-6 h-6 border-2 border-white/40 border-t-[#f97316] rounded-full animate-spin" />
                  </div>
                )}
              </div>
