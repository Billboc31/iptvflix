...
export interface ScoredCandidate {
  ...
  confidence: number   // clamped [0, 1] — for storage
  rawScore: number     // titleScore + yearScore unclamped — used for gap detection
  titleScore: number
  yearScore: number
}
...
function computeTitleScore(query: string, candidate: string): number {
...
export function scoreCandidates(
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.mediaType !== input.mediaType) continue
