...
function resolvePlaylistUrl(playlistUrl: string, username?: string, password?: string): string {
  let resolved = playlistUrl
  if (username !== undefined) resolved = resolved.replaceAll('{username}', encodeURIComponent(username))
  if (password !== undefined) resolved = resolved.replaceAll('{password}', encodeURIComponent(password))
  return resolved
}
...
export class M3UClient {
  ...
  async fetchSnapshot(sourceId: string): Promise<M3UCatalogSnapshot> {
    let response: Response
    try {
      response = await globalThis.fetch(this.resolvedUrl, {
        signal: AbortSignal.timeout(this.timeoutMs),
