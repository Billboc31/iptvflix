...
import { getEmbeddingIndexMode, setEmbeddingIndexMode, type EmbeddingIndexMode } from './embedding-index-mode.js'
 ...
 */
export async function ensurePgvectorEmbeddings(): Promise<EmbeddingIndexMode> {
  try {
    const available = await db.execute(
      sql`SELECT 1 FROM pg_available_extensions WHERE name = 'vector' LIMIT 1`,
    )
