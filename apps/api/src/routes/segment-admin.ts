...
export async function segmentAdminRoutes(app: FastifyInstance): Promise<void> {
  app.get('/admin/segments/coverage', async () => {
    ...
    const [selectionRow] = await db.select({
      withSelection: sql<number>`cast(count(distinct episode_id) as integer)`,
    ...
    const providerEpisodeCounts = await db
      .select({
        ...
        episodeCount: sql<number>`cast(count(distinct episode_id) as integer)`,
      })
      .from(mediaSegments)
    ...
    const byProvider: Record<string, { episodes: number; byType: Record<string, number> }> = {}
    ...
    for (const row of providerTypeCounts) {
      if (!byProvider[row.provider]) byProvider[row.provider] = { episodes: 0, byType: {} }
    ...
    `)
    const overlapCount = Number((overlapResult as unknown as Array<Record<string, unknown>>)[0]?.overlap ?? 0)
    ...
    `)
    const disagreeCount = Number((disagreeResult as unknown as Array<Record<string, unknown>>)[0]?.disagree ?? 0)
