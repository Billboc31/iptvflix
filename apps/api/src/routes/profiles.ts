...
import type { CreateProfileBody, UpdateProfileBody } from '@iptvflix/api-contracts'
import {
  listProfiles,
  createProfile,
  updateProfile,
...
export async function profilesRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Body: CreateProfileBody }>('/profiles', async (request, reply) => {
    if (!request.account) {
      return reply.status(401).send({ error: 'Unauthorized' })
    }
    ...
    try {
      ...
      if ((err as Error & { code?: string }).code === 'PROFILE_LIMIT_REACHED') {
  ...
  app.patch<{ Params: { profileId: string }; Body: UpdateProfileBody }>(
    '/profiles/:profileId',
    async (request, reply) => {
      if (!request.account) {
        return reply.status(401).send({ error: 'Unauthorized' })
  ...
  app.post<{ Params: { profileId: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if ((err as Error & { code?: string }).code === 'PROFILE_NOT_FOUND') {
