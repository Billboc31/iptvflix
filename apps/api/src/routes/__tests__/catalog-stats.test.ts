...
describe('GET /admin/catalog-stats', () => {
  it('returns valid shape with correct field structure', async () => {
    // 8 queries: movieStats, seriesStats, episodeCount, movieAvail, seriesAvail, episodeAvail,
    // oldestMovieSync, oldestSeriesSync
    setupSelectFrom({ total: 150, withAvailability: 40, upcoming: 10, enriched: 80 })
    setupSelectFrom({ total: 60, withAvailability: 20, upcoming: 5, enriched: 30 })
    ...
    expect(body.movies.enriched).toBe(80)
    expect(body.movies.embeddingPending).toBe(0)

    expect(body.series.total).toBe(60)
    expect(body.series.withoutAvailability).toBe(40) // 60 - 20
    expect(body.series.embeddingPending).toBe(0)
  ...
  it('withoutAvailability is positive when catalog has titles with no availability', async () => {
    setupSelectFrom({ total: 200, withAvailability: 50, upcoming: 0, enriched: 0 })
    setupSelectFrom({ total: 80, withAvailability: 10, upcoming: 0, enriched: 0 })
