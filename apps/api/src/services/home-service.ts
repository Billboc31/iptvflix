...
export async function buildHome(profileId: string): Promise<HomeResponse> {
  ...
  const forYouItems: ShelfItem[] = available.slice(0, 20).map(candidateToItem)
  const upcomingItems: ShelfItem[] = upcoming.slice(0, 10).map(candidateToItem)
...
function candidateToItem(c: RecommendationCandidate): ShelfItem {
  return {
