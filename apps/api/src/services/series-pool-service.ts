...
export async function getOrCreateSeriesSession(
  profileId: string,
): Promise<{ id: string; profileId: string; cursorReference: string | null }> {
  const now = new Date()
  const existing = await db
    ...
    .where(
      and(
        eq(recommendationSeriesSessions.profileId, profileId),
        sql`${recommendationSeriesSessions.expiresAt} > ${now.toISOString()}::timestamptz`,
      ),
    )
...
export async function countUnservedSeries(sessionId: string): Promise<number> {
  const [row] = await db
    .select({ n: count() })
    .from(shelfInstances)
...
export async function serveSeriesBatch(
  sessionId: string,
  nextPosition: number,
  batchSize: number = SERIES_BATCH_SIZE,
): Promise<{
  shelves: Array<{ instanceId: string; title: string; verticalPosition: number; items: Array<{ mediaType: string; mediaId: string }> }>
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(shelfInstances.seriesSessionId, sessionId),
...
export function fillSeriesPool(sessionId: string, profileId: string, targetCount: number): void {
...
export async function fillSeriesPoolAsync(sessionId: string, profileId: string, targetCount: number): Promise<void> {
  return _fillSeriesPoolAsync(sessionId, profileId, targetCount)
}

async function _fillSeriesPoolAsync(sessionId: string, profileId: string, targetCount: number): Promise<void> {
  ...
  const candidateConcepts = conceptRows.filter((c) => {
    if (usedConceptIds.has(c.id)) return false
    const types = (c.desiredMediaTypes as string[]) ?? []
...
async function getFreshSeriesIds(seriesIds: string[], cutoffDate: Date): Promise<Set<string>> {
...
async function getInProgressSeriesIds(profileId: string): Promise<Set<string>> {
  try {
    const rows = await db
      ...
      .where(
        and(
          ...
          gt(viewingProgress.progressSeconds, IN_PROGRESS_MIN_SECONDS),
        ),
      )
...
async function buildSeriesEnrichmentMap(
  seriesIds: string[],
...
async function selectExploitationConcept(
  profileId: string,
  sessionId: string,
...
async function selectExplorationConcept(
  profileId: string,
  fatigueService: ShelfFatigueService,
  usedConceptIds: Set<string>,
): Promise<typeof shelfConcepts.$inferSelect | null> {
  const conceptRows = await db
    ...
    .where(
      and(
        eq(shelfConcepts.profileId, profileId),
        eq(shelfConcepts.active, true),
        eq(shelfConcepts.generationType, 'EXPLORATION'),
      ),
    )
    .orderBy(desc(shelfConcepts.createdAt))
    .limit(10)
...
// ---------------------------------------------------------------------------
// Declared rails — first-render personalized Series sequence
// ---------------------------------------------------------------------------
...
export async function buildSeriesDeclaredRails(
  ...
  const excludedMediaIds = new Set<string>(inProgressIds)
  ...
  if (pendingRails.length === 0) return { shelves: results, nextPoolPosition: nextPosition, shelfInstanceIds: [] }
    ...
    try {
      ...
      results.push({
        ...
        items: rail.candidates.map((c): ShelfItem => ({
          mediaType: c.mediaType as 'SERIES',
          mediaId: c.mediaId,
...
export async function buildSeriesFallbackShelf(): Promise<ShelfResponse> {
  const rows = await db
