...
export async function recordArrivalsForFollowers(
...
export async function listArrivals(
  ...
  const srcIds = [...new Set(rows.map((r) => r.sourceId).filter((id): id is string => id !== null))]
  ...
  return rows.map((row) => ({
    ...
    mediaId: row.mediaId,
    mediaTitle:
      (row.mediaType === 'MOVIE' ? movieTitles[row.mediaId] : seriesTitles[row.mediaId]) ?? 'Unknown',
...
export async function markRead(profileId: string, arrivalId: string): Promise<void> {
  const [row] = await db
    .select({ id: mediaArrivals.id })
    .from(mediaArrivals)
    .where(and(eq(mediaArrivals.id, arrivalId), eq(mediaArrivals.profileId, profileId)))

  if (!row) throw new NotFoundError('Arrival', arrivalId)
