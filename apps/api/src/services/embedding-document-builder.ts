...
function popularityBucket(popularity: number | null | undefined): string | null {
  if (popularity == null) return null
  if (popularity >= 100) return 'très populaire'
  if (popularity >= 30) return 'populaire'
  if (popularity >= 10) return 'modéré'
  return null
}

export function buildMovieEmbeddingDocument(
  movie: MovieFields,
  genres: Genre[],
  credits: MediaCredit[],
): EmbeddingDocument {
  ...
  if (movie.voteAverage != null) ratingParts.push(`rating ${movie.voteAverage.toFixed(1)}/10`)
  if (bucket) ratingParts.push(bucket)
...
export function buildSeriesEmbeddingDocument(
  series: SeriesFields,
  genres: Genre[],
  credits: MediaCredit[],
): EmbeddingDocument {
  ...
  const ratingParts: string[] = []
  if (series.voteAverage != null) ratingParts.push(`rating ${series.voteAverage.toFixed(1)}/10`)
  if (bucket) ratingParts.push(bucket)
  if (ratingParts.length > 0) {
    lines.push(`Popularity/rating: ${ratingParts.join(', ')}`)
  }
...
export function measureCoverage(
  ...
  const hasYear = Boolean(
    'year' in media ? media.year : 'firstAirYear' in media ? (media as SeriesFields).firstAirYear : false,
  )
