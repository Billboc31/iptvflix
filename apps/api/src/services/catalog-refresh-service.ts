...
import type { MetadataProvider, MetadataCandidate, DiscoveryFeed } from '../providers/metadata/types.js'
import type { MetadataEnrichmentService } from './metadata-enrichment-service.js'
import {
  CATALOG_REFRESH_UPCOMING_STALE_HOURS,
  CATALOG_REFRESH_RECENT_STALE_DAYS,
  CATALOG_REFRESH_STABLE_STALE_DAYS,
  CATALOG_REFRESH_DISCOVERY_MAX_PAGES,
} from '../config/env.js'
...
export function classifyMovieBucket(
  movie: { status: string | null; theatricalReleaseDate: string | null },
  now: Date = new Date(),
): ContentBucket {
  const cutoff60 = new Date(+now - 60 * 86_400_000)
  const cutoff90 = new Date(+now - 90 * 86_400_000)
  if (
    (movie.status !== null && MOVIE_UPCOMING_STATUSES.includes(movie.status)) ||
...
export function classifySeriesBucket(show: { status: string | null }): ContentBucket {
  if (show.status !== null && SERIES_UPCOMING_STATUSES.includes(show.status)) return 'upcoming'
  return 'stable'
}
...
const DISCOVERY_FEEDS: DiscoveryFeed[] = ['upcoming', 'trending']
...
export class CatalogRefreshService {
  ...
  async run(): Promise<string> {
    ...
    await this.db
      ...
      .where(
        and(
          eq(catalogRefreshRuns.status, 'RUNNING'),
          lt(
            catalogRefreshRuns.startedAt,
            new Date(Date.now() - STALE_LOCK_HOURS * 3_600_000),
          ),
        ),
      )
    ...
    if (running.length > 0) throw new CatalogRefreshAlreadyRunningError()
  ...
  private async execute(runId: string): Promise<void> {
    try {
      ...
      const checkpoint: RefreshCheckpoint = (runRow?.checkpoint as RefreshCheckpoint) ?? {}
  ...
  private async runRefreshBucket(opts: {
    runId: string
    key: string
    ...
    for (;;) {
      ...
      for (const entity of entities) {
        try {
          ...
          if (result === 'enriched') {
            if (mediaType === 'MOVIE') counts.moviesRefreshed++
            else counts.seriesRefreshed++
  ...
  private async runDiscoveryFeed(opts: {
    runId: string
    key: string
    mediaType: 'MOVIE' | 'SERIES'
    feed: DiscoveryFeed
  ...
  private async fetchStaleMovies(
    bucket: ContentBucket,
    staleDays: number,
    offset: number,
  ): Promise<{ id: string }[]> {
    ...
    if (bucket === 'upcoming') {
      bucketWhere = or(
        inArray(movies.status, MOVIE_UPCOMING_STATUSES),
        sql`${movies.theatricalReleaseDate} > CURRENT_DATE - INTERVAL '60 days'`,
      )
    } else if (bucket === 'recent') {
      bucketWhere = and(
        or(isNull(movies.status), sql`${movies.status} NOT IN ('Rumored', 'Planned', 'In Production', 'Post Production')`),
        sql`${movies.theatricalReleaseDate} > CURRENT_DATE - INTERVAL '90 days'`,
        sql`${movies.theatricalReleaseDate} <= CURRENT_DATE - INTERVAL '60 days'`,
      )
    } else {
      bucketWhere = and(
        or(isNull(movies.status), sql`${movies.status} NOT IN ('Rumored', 'Planned', 'In Production', 'Post Production')`),
        or(isNull(movies.theatricalReleaseDate), sql`${movies.theatricalReleaseDate} <= CURRENT_DATE - INTERVAL '90 days'`),
      )
    }
  ...
  private async fetchStaleSeries(
    bucket: ContentBucket,
    staleDays: number,
    offset: number,
  ): Promise<{ id: string }[]> {
    ...
    if (bucket === 'upcoming') {
      ...
      bucketWhere = and(
        or(isNull(series.status), sql`${series.status} NOT IN ('In Production', 'Planned')`),
        sql`${series.theatricalReleaseDate} > CURRENT_DATE - INTERVAL '90 days'`,
      )
    } else {
      bucketWhere = and(
        or(isNull(series.status), sql`${series.status} NOT IN ('In Production', 'Planned')`),
        or(isNull(series.theatricalReleaseDate), sql`${series.theatricalReleaseDate} <= CURRENT_DATE - INTERVAL '90 days'`),
      )
    }
  ...
  private async updateRun(
    runId: string,
    data: Partial<RunCounts & { checkpoint?: RefreshCheckpoint }>,
  ): Promise<void> {
    await this.db.update(catalogRefreshRuns).set(data).where(eq(catalogRefreshRuns.id, runId))
  }

  async getLastCompletedRun(): Promise<{ completedAt: Date | null } | undefined> {
    const [run] = await this.db
