...
import { mergeSegments, type MergedSegment } from './segment-merger.js'
...
export class SegmentSyncService {
  ...
  async upsertSelections(episodeId: string, selections: MergedSegment[]): Promise<void> {
    ...
    const sorted = [...selections].sort((a, b) => {
      ...
      if (aProviders !== bProviders) return aProviders - bProviders  // fewer = worse = first
      ...
      if (aSubm !== bSubm) return aSubm - bSubm  // lower = worse = first
  ...
  async syncEpisode(
    ...
    let seriesTmdbId: number | null = null
    try {
