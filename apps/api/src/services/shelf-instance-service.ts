...
import type {
  ShelfInstanceDetail,
  ShelfInstanceItemDetail,
  PersistShelfInstanceParams,
...
export class ShelfInstanceService {
  ...
  async persistShelfInstance(params: PersistShelfInstanceParams): Promise<string> {
    return await this.db.transaction(async (tx) => {
      ...
      if (params.items.length > 0) {
        await tx.insert(shelfInstanceItems).values(
  ...
  async markFirstDisplayed(shelfInstanceId: string, at: Date = new Date()): Promise<void> {
  ...
  async markItemVisible(
    ...
    await this.db
      ...
      .where(
        and(
          eq(shelfInstanceItems.shelfInstanceId, shelfInstanceId),
          eq(shelfInstanceItems.mediaId, mediaId),
          eq(shelfInstanceItems.mediaType, mediaType),
        ),
      )
  }

  async markItemOpened(
    shelfInstanceId: string,
    mediaId: string,
    mediaType: string,
    at: Date,
    ...
    await this.db
      ...
      .where(
        and(
          ...
          eq(shelfInstanceItems.mediaId, mediaId),
          eq(shelfInstanceItems.mediaType, mediaType),
          isNull(shelfInstanceItems.openedAt),
        ),
      )
    ...
    await this.db
      ...
      .where(
        and(
          eq(profileMediaExposure.profileId, instance.profileId),
          eq(profileMediaExposure.mediaType, mediaType),
          eq(profileMediaExposure.mediaId, mediaId),
        ),
      )
  }
    ...
    await this.db
      ...
      .where(
        and(
          ...
          eq(shelfInstanceItems.mediaId, mediaId),
          eq(shelfInstanceItems.mediaType, mediaType),
          isNull(shelfInstanceItems.playedAt),
        ),
      )
  ...
  async getShelfInstanceWithItems(id: string): Promise<ShelfInstanceDetail | null> {
    const [instance] = await this.db
      .select()
      .from(shelfInstances)
    ...
    return {
      ...
      generationReasonCodes: (instance.generationReasonCodes as string[]) ?? [],
  ...
  async listProfileShelfInstances(
    profileId: string,
    limit: number,
    before?: string,
  ): Promise<ShelfInstanceDetail[]> {
    let query = this.db
      .select()
      .from(shelfInstances)
    ...
    const itemsByInstance = new Map<string, typeof allItems>()
