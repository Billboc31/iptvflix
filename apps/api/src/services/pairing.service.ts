...
export async function getPairingCodeDetail(code: string): Promise<PairingCodeDetailResponse> {
...
export async function getPairingStatus(code: string): Promise<PairingStatusResponse> {
  const [row] = await db
    .select()
    .from(pairingCodes)
