...
export async function runCompaction(): Promise<{ deleted: number; anonymized: number }> {
  const now = Date.now()
  ...
  const analyticsDeleted = await db
    ...
    .where(
      and(
        inArray(profileInteractionEvents.eventType, ANALYTICS_TYPES),
  ...
  const standardDeleted = await db
    ...
    .where(
      and(
        inArray(profileInteractionEvents.eventType, STANDARD_TYPES),
