...
export function buildXtreamMovieUrl(baseUrl, username, password, providerItemId, containerExtension) {
...
export function buildXtreamEpisodeUrl(baseUrl, username, password, providerItemId, containerExtension) {
    const base = baseUrl.replace(/\/$/, '');
    const ext = containerExtension || 'ts';
    return `${base}/series/${username}/${password}/${providerItemId}.${ext}`;
}
/** HTTPS so a HTTPS web app can play without mixed-content blocking. */
export function browserSafeXtreamUrl(url) {
    try {
...
export const XTREAM_STREAM_HEADERS = {
 ...
 */
export function xtreamUrlFallbacks(url) {
    ...
    try {
        ...
        const pathVariants = [];
        if (seriesIdx >= 0) {
            for (const e of alts.slice(0, 3))
                pathVariants.push(`/series/${user}/${pass}/${id}.${e}`);
        }
        else {
            for (const e of alts.slice(0, 3))
                pathVariants.push(`/movie/${user}/${pass}/${id}.${e}`);
...
export async function resolveXtreamFetchTarget(url) {
    try {
        const parsed = new URL(url);
...
async function fetchCandidate(url, headers, signal) {
...
export async function fetchXtreamStream(url, headers, signal) {
    const candidates = xtreamUrlFallbacks(url);
    let lastResponse;
    let lastError;
    const attempts = [
        ...candidates.map((candidate) => ({ candidate, ua: XTREAM_USER_AGENTS[0] })),
        ...candidates.slice(0, 3).flatMap((candidate) => XTREAM_USER_AGENTS.slice(1).map((ua) => ({ candidate, ua }))),
    ];
    for (const { candidate, ua } of attempts) {
        try {
            const res = await fetchCandidate(candidate, { ...headers, 'User-Agent': ua }, signal);
            if (res.ok || res.status === 206)
                return res;
            lastResponse = res;
        }
...
export async function pickWorkingXtreamUrl(url, signal) {
    const candidates = xtreamUrlFallbacks(url);
...
export async function ffmpegInputArgs(providerUrl) {
