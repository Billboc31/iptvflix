...
export class MediaReconciliationService {
    ...
    constructor(titleMatchingService) {
        this.titleMatchingService = titleMatchingService;
    }
    ...
    async startRun(opts) {
        ...
        if (existing)
            throw new ReconciliationAlreadyRunningError(existing.id);
        try {
            const [run] = await db
    ...
    async _processType(type, runId, batchSize, concurrency, totals, dryRun) {
        ...
        const [currentRun] = await db
            ...
            .from(reconciliationRuns)
            .where(eq(reconciliationRuns.id, runId));
        let cursorId = type === 'MOVIE' ? (currentRun?.cursorMovieId ?? null) : (currentRun?.cursorSeriesId ?? null);
        while (true) {
            ...
            const rawResults = indexedInputs.length > 0
                ? await this.titleMatchingService.matchBatch(indexedInputs.map((i) => i.input), { concurrency, throttleMs: MATCH_THROTTLE_MS })
                : [];
            ...
            for (const media of page) {
                ...
                const canonicalIds = new Set(matchedResults
            ...
            if (!dryRun) {
                const cursorUpdate = type === 'MOVIE' ? { cursorMovieId: lastId } : { cursorSeriesId: lastId };
                await db
                    .update(reconciliationRuns)
                    .set({
                    ...cursorUpdate,
    ...
    async _fetchPage(type, cursorId, batchSize) {
        const table = type === 'MOVIE' ? movies : series;
    ...
    async _fetchAvailabilities(type, mediaIds) {
    ...
    async _reconcileMedia(oldId, canonicalId, type) {
    ...
    async _migrateMovieData(oldId, canonicalId, tx) {
    ...
    async _migrateSeriesData(oldId, canonicalId, tx) {
        // Availabilities
        await tx.execute(sql `
    ...
    async _migrateUserState(oldId, canonicalId, type, tx) {
...
//# sourceMappingURL=media-reconciliation-service.js.map