...
import { shelfInstances, shelfInstanceItems, movies, mediaVideos, } from '../db/schema/index.js';
import { signCursor, verifyCursor } from '../lib/home-cursor.js';
import { getOrCreateMoviesSession, getMoviesSessionById, countMoviesUnserved, serveMoviesBatch, buildMoviesDeclaredRails, fillMoviesPool, buildMoviesFallbackShelf, resolveMoviesNextServePosition, resolveMoviesAppendPosition, } from './movies-pool-service.js';
import { getMoviesSnapshot, saveMoviesSnapshot, isMoviesSnapshotValid, isMoviesSnapshotStale, } from './movies-snapshot-service.js';
import { resolveMediaImageUrl } from '../lib/tmdb-image.js';
import { MOVIES_BATCH_SIZE, MOVIES_POOL_MIN, MOVIES_POOL_TARGET, MOVIES_SNAPSHOT_TTL_HOURS, } from '../config/env.js';
// ---------------------------------------------------------------------------
// buildMoviesPage — cursor-based entry point
// ---------------------------------------------------------------------------
export async function buildMoviesPage(profileId, cursor) {
    // ── Cursor request ────────────────────────────────────────────────────────
    if (cursor) {
        ...
        const { shelves: batchRows, newNextPosition, hasMore } = await serveMoviesBatch(decoded.sessionId, decoded.nextPosition, MOVIES_BATCH_SIZE);
...
// ---------------------------------------------------------------------------
async function _regenerateMoviesSnapshot(profileId, sessionId) {
    try {
...
// ---------------------------------------------------------------------------
async function reconstructMoviesShelves(shelfInstanceIds) {
