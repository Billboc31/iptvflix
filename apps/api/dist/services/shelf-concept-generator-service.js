...
import { shelfConcepts, profileInteractionEvents, profiles, discoveryCandidate, movies, series as seriesTable, } from '../db/schema/index.js';
...
import { SHELF_CONCEPT_PERSONALIZED_RATIO, SHELF_CONCEPT_EXPLORATION_RATIO, SHELF_CONCEPT_DISCOVERY_RATIO, SHELF_CONCEPT_BATCH_SIZE, SHELF_CONCEPT_TTL_HOURS, SHELF_CONCEPT_MIN_POOL_SIZE, SHELF_CONCEPT_SEMANTIC_DEDUP_THRESHOLD, } from '../config/env.js';
const PROMPT_VERSION = 'shelf-concept-v1';
const COLD_START_THRESHOLD = 3;
const MAX_STRING_LEN = 100;
const ALLOWED_GEN_TYPES = new Set([
...
export class ShelfConceptGeneratorService {
    ...
    semanticRetrieval;
    model;
    constructor(db, openai, embeddingProvider, semanticRetrieval, model) {
        this.db = db;
        this.openai = openai;
    ...
    async buildProfileContext(profileId) {
        ...
        const seriesPlayEvents = interactionEvents.filter((e) => e.mediaType === 'SERIES' &&
    ...
    validateConcept(concept, existingConcepts) {
        ...
        if (!Array.isArray(concept.desiredMediaTypes) ||
            !concept.desiredMediaTypes.every((t) => ALLOWED_MEDIA_TYPES.has(t))) {
            return { valid: false, reason: 'invalid desiredMediaTypes' };
        }
        if (concept.freshnessPolicy !== null &&
    ...
    async generateConcepts(profileId, opts) {
        ...
        try {
            ...
            console.error('[ShelfConceptGeneratorService] LLM response not valid JSON (possibly truncated — check max_tokens):', content.slice(0, 200));
            return [];
        }
        let rawConcepts;
        if (parsed &&
        ...
        if (this.embeddingProvider && existingConcepts.length > 0) {
            const existingEmbeddings = await Promise.all(existingConcepts.map((c) => this.embeddingProvider.embed(c.semanticIntent.trim()).catch(() => null)));
        ...
        for (const raw of rawConcepts) {
            ...
            if (this.semanticRetrieval) {
                ...
                if (candidates.length < 3) {
                    console.warn(`[ShelfConceptGeneratorService] concept rejected (dry-run ${candidates.length} candidates):`, raw.title);
                    continue;
                }
            }
...
//# sourceMappingURL=shelf-concept-generator-service.js.map