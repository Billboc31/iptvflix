...
export class ExternalDiscoveryService {
    db;
    provider;
    movieQueryCache = new Map();
    seriesQueryCache = new Map();
    constructor(db, provider) {
        this.db = db;
        this.provider = provider;
    }
    async discoverMovies(query, excludeTmdbIds) {
        const now = Date.now();
    ...
    async discoverSeries(query, excludeTmdbIds) {
        const now = Date.now();
    ...
    async materializeMovie(tmdbId) {
        ...
        if (!Number.isInteger(numericId) || numericId <= 0) {
            const err = new Error(`Invalid tmdbId: ${tmdbId}`);
            err.statusCode = 409;
            throw err;
        }
...
//# sourceMappingURL=external-discovery-service.js.map