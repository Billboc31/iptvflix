...
import { shelves, shelfMembers, movies, series, movieGenres, seriesGenres, discoveryCandidate, } from '../db/schema/index.js';
...
// ---------------------------------------------------------------------------
async function materializeDiscoveryCandidate(candidateId, mediaType) {
    ...
    if (mediaType === 'MOVIE') {
        if (dc.canonicalMovieId)
            return dc.canonicalMovieId;
        ...
        if (dc.canonicalSeriesId)
            return dc.canonicalSeriesId;
...
async function resolveGeneratedMembers(profileId, seedMediaIds, opts) {
    ...
    const inferredGenreIdSet = new Set();
    for (const r of movieGenreRows)
        inferredGenreIdSet.add(r.genreId);
    for (const r of seriesGenreRows)
        inferredGenreIdSet.add(r.genreId);
...
// ---------------------------------------------------------------------------
export async function generateShelfFromSeeds(profileId, body) {
...
// ---------------------------------------------------------------------------
export async function refreshGeneratedShelf(shelfId, profileId) {
...
//# sourceMappingURL=shelf-generation-service.js.map