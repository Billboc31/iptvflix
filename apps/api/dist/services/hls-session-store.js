...
cleanupTimer.unref();
async function expireSession(id, session) {
    sessions.delete(id);
 ...
 */
export async function createHlsSession(sessionId, providerUrl, mode, startPositionSeconds = 0) {
...
export function getHlsSession(sessionId) {
...
export async function waitForPlaylist(sessionId, timeoutMs = 15_000) {
...
export async function getSegment(sessionId, filename) {
    if (!SEGMENT_RE.test(filename))
    ...
    if (!segmentPath.startsWith(entry.tempDir + '/'))
        return { status: 'invalid' };
    return { status: 'ok', filePath: segmentPath };
}
export function getHlsSessionInfo(sessionId) {
    const entry = getHlsSession(sessionId);
    if (!entry)
        return null;
    return { tempDir: entry.tempDir, createdAt: entry.createdAt };
}
//# sourceMappingURL=hls-session-store.js.map