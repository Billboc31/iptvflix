export function classifyDelivery(mediaInfo) {
    ...
    if (isH264 && isAAC && isMp4Container)
        return 'DIRECT';
...
export function insertSeekBeforeInput(args, startPositionSeconds) {
    const start = Math.floor(startPositionSeconds);
    if (start <= 30)
        return args;
    const i = args.indexOf('-i');
    if (i < 0)
        return ['-ss', String(start), ...args];
    return [...args.slice(0, i), '-ss', String(start), ...args.slice(i)];
}
export function buildFfmpegArgs(mode, tempDir) {
...
//# sourceMappingURL=playback-compat.js.map