...
import { movies, movieAvailabilities, movieGenres, series, seriesAvailabilities, seriesGenres, genres, } from '../db/schema/index.js';
...
export class SimilarTitlesService {
    ...
    async getSimilarMovies(movieId, limit = 20) {
        ...
        try {
            ...
            const candidates = mergeCandidates([...similar.results, ...recommendations.results], source.tmdbId);
    ...
    async getSimilarSeries(seriesId, limit = 20) {
        const [source] = await this.db
    ...
    async resolveMovieCandidates(candidates, sourceId) {
        ...
        const allLocalRows = toMaterialize.length > 0
            ? await this.db
                .select(localFields)
                .from(movies)
                .where(inArray(movies.tmdbId, candidateTmdbIds))
    ...
    async resolveSeriesCandidates(candidates, sourceId) {
        if (candidates.length === 0)
            return [];
        const candidateTmdbIds = candidates.map((c) => c.tmdbId);
        const localFields = {
    ...
    async fallbackMoviesByGenre(sourceId, limit) {
    ...
    async fallbackSeriesByGenre(sourceId, limit) {
        const genreRows = await this.db
    ...
    async buildMovieAvailabilitySet(ids) {
        if (ids.length === 0)
            return new Set();
    ...
    async buildSeriesAvailabilitySet(ids) {
        if (ids.length === 0)
            return new Set();
        const rows = await this.db
    ...
    async upsertMovieGenres(movieId, genreObjects) {
    ...
    async materializeSeries(tmdbId) {
        const [existing] = await this.db
            .select({ id: series.id })
            .from(series)
    ...
    async upsertSeriesGenres(seriesId, genreObjects) {
        const genreValues = genreObjects.map((g) => ({
            name: g.name,
            slug: slugify(g.name),
            tmdbId: g.tmdbId,
...
function mergeCandidates(items, sourceTmdbId) {
    const seen = new Set([sourceTmdbId]);
    const result = [];
...
//# sourceMappingURL=similar-titles-service.js.map