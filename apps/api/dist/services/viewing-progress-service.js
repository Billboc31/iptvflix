...
export async function listContinueWatching(profileId) {
    const rows = await db
        ...
        .where(and(eq(viewingProgress.profileId, profileId), sql `${viewingProgress.progressSeconds} >= ${viewingProgress.durationSeconds} * 0.05`, sql `${viewingProgress.progressSeconds} < ${viewingProgress.durationSeconds} * 0.90`))
