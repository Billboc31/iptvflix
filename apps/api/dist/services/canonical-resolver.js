...
export class CanonicalResolver {
    enrichmentService;
    onNewEpisode;
    constructor(enrichmentService, onNewEpisode) {
        this.enrichmentService = enrichmentService;
        this.onNewEpisode = onNewEpisode;
    }
    async resolveMovieCanonical(input) {
    ...
    async resolveSeriesCanonical(input) {
        if (input.tmdbId != null) {
            const cached = input.tmdbCache?.get(input.tmdbId);
            if (cached)
                return { id: cached };
     ...
     */
    async resolveEpisodeCanonical(input) {
        let seasonId;
        ...
        if (inserted[0]) {
            if (this.onNewEpisode)
                this.onNewEpisode(inserted[0].id);
            return inserted[0];
        }
        const [row] = await db
...
//# sourceMappingURL=canonical-resolver.js.map