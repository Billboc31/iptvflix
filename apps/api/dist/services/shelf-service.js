...
import { shelves, shelfMembers, movies, series, movieAvailabilities, seriesAvailabilities, viewingProgress, movieGenres, seriesGenres, } from '../db/schema/index.js';
...
// ---------------------------------------------------------------------------
async function resolveSystemShelf(id, profileId) {
    ...
    if (id === 'sys_recently_added_movies') {
        const rows = await db
            ...
            .limit(20);
        return rows.map((r) => ({ mediaType: 'MOVIE', mediaId: r.id, title: r.title, posterUrl: r.posterPath }));
    }
    if (id === 'sys_recently_added_series') {
        const rows = await db
            ...
            .limit(20);
        return rows.map((r) => ({ mediaType: 'SERIES', mediaId: r.id, title: r.title, posterUrl: r.posterPath }));
    }
    return [];
}
// ---------------------------------------------------------------------------
...
export function validateDynamicRules(rawRules) {
    ...
    if ('genreIds' in input) {
        if (!Array.isArray(input.genreIds) || !input.genreIds.every((id) => typeof id === 'string')) {
            throw new ValidationError('genreIds must be an array of strings');
        }
        result.genreIds = input.genreIds;
    }
...
// ---------------------------------------------------------------------------
async function evaluateMovies(rules, profileId) {
    const conditions = [
        ...
        rules.genreIds?.length
            ? inArray(movies.id, db.select({ id: movieGenres.movieId }).from(movieGenres).where(inArray(movieGenres.genreId, rules.genreIds)))
            : undefined,
        ...
        rules.watchState === 'UNWATCHED'
            ? notInArray(movies.id, db
                .select({ id: viewingProgress.mediaId })
                .from(viewingProgress)
                .where(and(eq(viewingProgress.profileId, profileId), eq(viewingProgress.mediaType, 'MOVIE'), sql `${viewingProgress.progressSeconds} >= ${viewingProgress.durationSeconds} * 0.05`)))
            : undefined,
    ].filter(Boolean);
    const rows = await db
        .select({ id: movies.id, title: movies.title, posterPath: movies.posterPath })
...
async function evaluateSeries(rules) {
    const conditions = [
...
export async function evaluateDynamicShelf(rules, profileId) {
    if (rules.mediaType === 'MOVIE')
        return evaluateMovies(rules, profileId);
    if (rules.mediaType === 'SERIES')
        return evaluateSeries(rules);
...
// ---------------------------------------------------------------------------
async function resolveManualItems(shelfId) {
    ...
    return members.map((m) => {
        if (m.mediaType === 'MOVIE') {
            ...
            return { mediaType: 'MOVIE', mediaId: m.mediaId, title: meta?.title ?? m.mediaId, posterUrl: meta?.posterPath ?? null };
        }
        else {
            const meta = seriesMap.get(m.mediaId);
            return { mediaType: 'SERIES', mediaId: m.mediaId, title: meta?.title ?? m.mediaId, posterUrl: meta?.posterPath ?? null };
        }
    });
}
// ---------------------------------------------------------------------------
// Public API
// ---------------------------------------------------------------------------
export async function listShelves(profileId) {
...
export async function getShelf(shelfId, profileId) {
    // System shelf
    ...
    return {
        ...
        title: shelf.title,
        type: shelf.type,
        layoutHint: shelf.layoutHint,
        items,
        ...(shelf.type === 'GENERATED'
            ? { generatedAt: shelf.rules.generatedAt }
            : {}),
    };
}
export async function createShelf(profileId, body) {
    const rules = body.type === 'DYNAMIC' ? validateDynamicRules(body.rules ?? {}) : undefined;
    const [{ maxPos }] = await db
        .select({ maxPos: sql `coalesce(max(${shelves.position}), -1)` })
        .from(shelves)
        .where(eq(shelves.profileId, profileId));
...
export async function updateShelf(shelfId, profileId, body) {
    if (isSystemId(shelfId))
        throw new ForbiddenError('Cannot modify system shelves');
    const [shelf] = await db.select().from(shelves).where(eq(shelves.id, shelfId));
    if (!shelf)
        throw new NotFoundError('Shelf', shelfId);
    if (shelf.profileId !== profileId)
...
export async function deleteShelf(shelfId, profileId) {
    if (isSystemId(shelfId))
        throw new ForbiddenError('Cannot delete system shelves');
    const [shelf] = await db.select().from(shelves).where(eq(shelves.id, shelfId));
    if (!shelf)
        throw new NotFoundError('Shelf', shelfId);
    if (shelf.profileId !== profileId)
...
export async function addMember(shelfId, profileId, body) {
    if (isSystemId(shelfId))
        throw new ForbiddenError('Cannot add members to system shelves');
    const [shelf] = await db.select().from(shelves).where(eq(shelves.id, shelfId));
    if (!shelf)
        throw new NotFoundError('Shelf', shelfId);
    if (shelf.profileId !== profileId)
    ...
    if (shelf.type !== 'MANUAL')
        throw new ValidationError('Can only add members to MANUAL shelves');
    // Validate media exists
    if (body.mediaType === 'MOVIE') {
        const [row] = await db.select({ id: movies.id }).from(movies).where(eq(movies.id, body.mediaId));
        if (!row)
            throw new NotFoundError('Movie', body.mediaId);
    }
    else {
        const [row] = await db.select({ id: series.id }).from(series).where(eq(series.id, body.mediaId));
        if (!row)
            throw new NotFoundError('Series', body.mediaId);
    }
    const [{ maxPos }] = await db
        .select({ maxPos: sql `coalesce(max(${shelfMembers.position}), -1)` })
        .from(shelfMembers)
        .where(eq(shelfMembers.shelfId, shelfId));
    const position = (maxPos ?? -1) + 1;
    await db
...
export async function removeMember(shelfId, profileId, mediaType, mediaId) {
    if (isSystemId(shelfId))
        throw new ForbiddenError('Cannot remove members from system shelves');
    const [shelf] = await db.select().from(shelves).where(eq(shelves.id, shelfId));
    if (!shelf)
        throw new NotFoundError('Shelf', shelfId);
    if (shelf.profileId !== profileId)
    ...
    await db
        .delete(shelfMembers)
        .where(and(eq(shelfMembers.shelfId, shelfId), eq(shelfMembers.mediaType, mediaType), eq(shelfMembers.mediaId, mediaId)));
}
export async function reorderMembers(shelfId, profileId, orderedList) {
    if (isSystemId(shelfId))
        throw new ForbiddenError('Cannot reorder members of system shelves');
    const [shelf] = await db.select().from(shelves).where(eq(shelves.id, shelfId));
    if (!shelf)
        throw new NotFoundError('Shelf', shelfId);
    if (shelf.profileId !== profileId)
    ...
    await db.transaction(async (tx) => {
        for (const [index, member] of orderedList.entries()) {
            await tx
                .update(shelfMembers)
                .set({ position: index })
                .where(and(eq(shelfMembers.shelfId, shelfId), eq(shelfMembers.mediaType, member.mediaType), eq(shelfMembers.mediaId, member.mediaId)));
        }
    });
}
