...
import { buildMovieEmbeddingDocument, buildSeriesEmbeddingDocument, hashDocument, } from './embedding-document-builder.js';
...
export class EmbeddingService {
    ...
    async upsertEmbedding(mediaId, mediaType) {
        ...
        const [existing] = await this.db
            ...
            .where(and(eq(mediaEmbeddings.mediaId, mediaId), eq(mediaEmbeddings.mediaType, mediaType), eq(mediaEmbeddings.modelProvider, this.provider.modelProvider), eq(mediaEmbeddings.modelName, this.provider.modelName)));
    ...
    async semanticSearch(queryText, topK) {
        ...
        const distanceExpr = usePgvector
            ? sql `${mediaEmbeddings.embedding} <=> ${sql.raw(vectorLiteral)}`
            : sql `(
          1.0 - (
        ...
        const rows = await this.db
            ...
            .from(mediaEmbeddings)
            .where(and(eq(mediaEmbeddings.modelProvider, this.provider.modelProvider), eq(mediaEmbeddings.modelName, this.provider.modelName)))
    ...
    async buildDocumentForMedia(mediaId, mediaType) {
        if (mediaType === 'MOVIE') {
            ...
            const doc = buildMovieEmbeddingDocument({ ...movie, collectionName: collectionRow[0]?.name ?? null }, genreRows, creditRows);
