...
function computeConfidence(entryTvgId, entryNormalized, candidate) {
    let confidence = 0;
    const provenance = { candidateId: candidate.id };
...
export const ChannelSyncService = {
    async syncLiveChannels(sourceId, entries, opts) {
        const result = {
            channelsCreated: 0,
            channelsUpdated: 0,
            sourcesCreated: 0,
        ...
        const existingSourcesByItemId = new Map();
        for (const row of existingSources) {
            existingSourcesByItemId.set(row.providerItemId, row);
        }
        const allChannels = await db.select().from(channels);
        const channelsByTvgId = new Map();
        const channelsByNormalizedName = new Map();
        ...
        const seenProviderItemIds = new Set();
        ...
        if (!opts?.skipLifecycle) {
            const missingProviderItemIds = [...existingSourcesByItemId.keys()].filter((id) => !seenProviderItemIds.has(id));
            if (missingProviderItemIds.length > 0) {
                await db
                    .update(channelSources)
                    .set({ status: 'UNAVAILABLE', unavailableAt: now })
                    .where(and(eq(channelSources.sourceId, sourceId), eq(channelSources.status, 'AVAILABLE'), inArray(channelSources.providerItemId, missingProviderItemIds)));
