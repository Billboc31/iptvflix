...
import { recommendationSeriesSessions, series, mediaVideos, shelfInstances, shelfInstanceItems, } from '../db/schema/index.js';
import { resolveMediaImageUrl } from '../lib/tmdb-image.js';
import { signCursor, verifyCursor } from '../lib/series-cursor.js';
import { getOrCreateSeriesSession, countUnservedSeries, serveSeriesBatch, buildSeriesDeclaredRails, fillSeriesPool, buildSeriesFallbackShelf, resolveSeriesNextServePosition, resolveSeriesAppendPosition, } from './series-pool-service.js';
import { getSeriesSnapshot, saveSeriesSnapshot, isSeriesSnapshotValid, isSeriesSnapshotStale, } from './series-snapshot-service.js';
import { SERIES_BATCH_SIZE, SERIES_POOL_MIN, SERIES_POOL_TARGET, SERIES_SNAPSHOT_TTL_HOURS, } from '../config/env.js';
// ---------------------------------------------------------------------------
// buildSeriesPage — cursor-based entry point
// ---------------------------------------------------------------------------
export async function buildSeriesPage(profileId, cursor) {
    // ── Cursor request ────────────────────────────────────────────────────────
    if (cursor) {
        ...
        const { shelves: batchRows, newNextPosition, hasMore } = await serveSeriesBatch(decoded.sessionId, decoded.nextPosition, SERIES_BATCH_SIZE);
    ...
    try {
        ...
        saveSeriesSnapshot(profileId, session.id, declared.shelfInstanceIds, expiresAt).catch((err) => {
...
// ---------------------------------------------------------------------------
async function _regenerateSeriesSnapshot(profileId, sessionId) {
    try {
...
// ---------------------------------------------------------------------------
async function reconstructShelvesFromSnapshot(shelfInstanceIds) {
...
//# sourceMappingURL=series-page-service.js.map