...
import { mediaVideos, recommendationHomeSessions, movies, series, shelfInstances, shelfInstanceItems, } from '../db/schema/index.js';
import { signCursor, verifyCursor } from '../lib/home-cursor.js';
import { getOrCreateSession, countUnserved, serveBatch, buildDeclaredRails, fillPool, buildFallbackShelf, } from './home-pool-service.js';
import { getSnapshot, saveSnapshot, isSnapshotValid, isStale, } from './home-snapshot-service.js';
import { getShelf } from './shelf-service.js';
import { HOME_BATCH_SIZE, HOME_POOL_MIN, HOME_POOL_TARGET, HOME_SNAPSHOT_TTL_HOURS, } from '../config/env.js';
// ---------------------------------------------------------------------------
// buildHome — cursor-based entry point
// ---------------------------------------------------------------------------
export async function buildHome(profileId, cursor) {
    // ── Cursor request ────────────────────────────────────────────────────────
    if (cursor) {
        ...
        const { shelves: batchRows, newNextPosition, hasMore } = await serveBatch(decoded.sessionId, decoded.nextPosition, HOME_BATCH_SIZE);
    ...
    try {
        ...
        saveSnapshot(profileId, session.id, declared.shelfInstanceIds, expiresAt, hero?.mediaId ?? null, hero?.mediaType ?? null).catch((err) => {
...
// ---------------------------------------------------------------------------
async function _regenerateSnapshot(profileId, sessionId) {
    try {
        const declared = await buildDeclaredRails(profileId, sessionId);
        const expiresAt = new Date(Date.now() + HOME_SNAPSHOT_TTL_HOURS * 60 * 60 * 1000);
        await saveSnapshot(profileId, sessionId, declared.shelfInstanceIds, expiresAt, declared.hero?.mediaId ?? null, declared.hero?.mediaType ?? null);
    }
    catch (err) {
...
// ---------------------------------------------------------------------------
async function reconstructShelvesFromSnapshot(profileId, shelfInstanceIds) {
    if (shelfInstanceIds.length === 0)
        return [];
    ...
    const generatedShelves = await batchRowsToShelfResponses(batchRows);
    const result = [];
    if (cwShelf && cwShelf.items.length > 0)
        result.push(cwShelf);
...
// ---------------------------------------------------------------------------
async function reconstructHero(heroMediaId, heroMediaType) {
    if (!heroMediaId || !heroMediaType)
        return null;
...
async function batchRowsToShelfResponses(batchRows) {
