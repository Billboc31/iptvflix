...
async function fetchAvailabilities(mediaType, mediaId) {
    if (mediaType === 'movie') {
        return db
            .select({
...
async function fetchProgress(profileId, mediaType, mediaId) {
    ...
    const [row] = await db
        ...
        .where(and(eq(viewingProgress.profileId, profileId), eq(viewingProgress.mediaType, pgMediaType), eq(viewingProgress.mediaId, mediaId)));
...
function extensionFallbackMode(containerExtension) {
    const ext = containerExtension.toLowerCase();
    if (ext === 'm3u8' || ext === 'm3u')
        return 'DIRECT';
    if (ext === 'ts' || ext === 'mkv' || ext === 'avi' || ext === 'flv' || ext === 'wmv')
        return 'HLS_REMUX';
...
export async function resolvePlayback(profileId, mediaType, mediaId, explicitAvailabilityId, correlationId = 'unknown', opts) {
    ...
    const candidates = allRows.filter((r) => r.status === 'AVAILABLE' && sourceMap.get(r.providerId)?.enabled === true);
    let selectedId;
    ...
    if (source.type === 'XTREAM') {
        ...
        if (mediaType === 'movie') {
            providerStreamUrl = buildXtreamMovieUrl(source.baseUrl, source.username ?? '', source.password ?? '', selected.providerItemId, containerExtension);
        }
        else {
            providerStreamUrl = buildXtreamEpisodeUrl(source.baseUrl, source.username ?? '', source.password ?? '', selected.providerItemId, containerExtension);
        }
...
function needsRelayRemux(ext) {
    const e = (ext || '').toLowerCase().replace(/^\./, '');
    return e === 'mkv' || e === 'ts' || e === 'm2ts' || e === 'avi' || e === '';
}
//# sourceMappingURL=playback-resolver.js.map