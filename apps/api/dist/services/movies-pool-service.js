...
import { shelfInstances, shelfInstanceItems, moviesSessions, shelfConcepts, movies, mediaVideos, movieAvailabilities, } from '../db/schema/index.js';
import { HOME_FRESH_DAYS, MOVIES_BATCH_SIZE, MOVIES_ITEMS_MAX, MOVIES_ITEMS_PER_SHELF, MOVIES_SESSION_TTL_HOURS, MOVIES_EXPLORATION_RATIO, } from '../config/env.js';
...
// Session management
// ---------------------------------------------------------------------------
export async function getOrCreateMoviesSession(profileId) {
    const now = new Date();
    const existing = await db
        .select()
        .from(moviesSessions)
        .where(and(eq(moviesSessions.profileId, profileId), sql `${moviesSessions.expiresAt} > ${now.toISOString()}::timestamptz`))
...
export async function getMoviesSessionById(sessionId) {
    const [row] = await db
        .select()
        .from(moviesSessions)
...
// Pool queries
// ---------------------------------------------------------------------------
export async function countMoviesUnserved(sessionId) {
    const [row] = await db
        .select({ n: count() })
        .from(shelfInstances)
...
export async function resolveMoviesNextServePosition(sessionId) {
...
export async function resolveMoviesAppendPosition(sessionId) {
    const [maxRow] = await db
        .select({ maxPos: sql `COALESCE(MAX(${shelfInstances.verticalPosition}), -1)` })
        .from(shelfInstances)
        .where(eq(shelfInstances.moviesSessionId, sessionId));
...
export async function serveMoviesBatch(sessionId, nextPosition, batchSize = MOVIES_BATCH_SIZE) {
    const rows = await db
        .select()
        .from(shelfInstances)
        .where(and(eq(shelfInstances.moviesSessionId, sessionId), isNull(shelfInstances.servedAt), sql `${shelfInstances.verticalPosition} >= ${nextPosition}`))
...
// Pool filling
// ---------------------------------------------------------------------------
export function fillMoviesPool(sessionId, profileId, targetCount) {
...
export async function fillMoviesPoolAsync(sessionId, profileId, targetCount) {
    return _fillMoviesPoolAsync(sessionId, profileId, targetCount);
}
async function _fillMoviesPoolAsync(sessionId, profileId, targetCount) {
    ...
    const conceptRows = await db
        .select()
        .from(shelfConcepts)
        .where(and(eq(shelfConcepts.profileId, profileId), eq(shelfConcepts.active, true), sql `${shelfConcepts.desiredMediaTypes} @> '["MOVIE"]'::jsonb`))
...
// ---------------------------------------------------------------------------
async function getFreshMovieIds(movieIds, cutoffDate) {
    ...
    const rows = await db
        .select({ id: movies.id })
        .from(movies)
        .where(and(inArray(movies.id, movieIds), or(gte(movies.theatricalReleaseDate, cutoffDateStr), gte(movies.digitalReleaseDate, cutoffDateStr))));
...
async function selectThematicMovieConcept(profileId, usedConceptIds, fatigueService) {
    const conceptRows = await db
        .select()
        .from(shelfConcepts)
        .where(and(eq(shelfConcepts.profileId, profileId), eq(shelfConcepts.active, true), eq(shelfConcepts.generationType, 'PERSONALIZED'), sql `${shelfConcepts.desiredMediaTypes} @> '["MOVIE"]'::jsonb`))
        .orderBy(desc(shelfConcepts.createdAt))
        .limit(20);
...
async function selectExplorationMovieConcept(profileId, usedConceptIds, fatigueService) {
    const conceptRows = await db
        .select()
        .from(shelfConcepts)
        .where(and(eq(shelfConcepts.profileId, profileId), eq(shelfConcepts.active, true), inArray(shelfConcepts.generationType, ['EXPLORATION', 'DISCOVERY']), sql `${shelfConcepts.desiredMediaTypes} @> '["MOVIE"]'::jsonb`))
        .orderBy(desc(shelfConcepts.createdAt))
        .limit(20);
    const candidates = conceptRows.filter((c) => !usedConceptIds.has(c.id));
    if (candidates.length === 0)
...
export async function buildMoviesDeclaredRails(profileId, sessionId, startPosition = 0) {
...
// ---------------------------------------------------------------------------
async function buildMovieEnrichmentMap(movieIds) {
    if (movieIds.length === 0)
        return new Map();
...
// ---------------------------------------------------------------------------
export async function buildMoviesFallbackShelf() {
    const rows = await db
...
//# sourceMappingURL=movies-pool-service.js.map