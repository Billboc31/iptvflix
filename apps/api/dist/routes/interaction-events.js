...
export async function interactionEventsRoutes(app) {
    app.post('/interaction-events', async (request, reply) => {
    ...
    app.post('/interaction-events/batch', async (request, reply) => {
        const body = request.body ?? {};
        ...
        for (const event of events) {
            try {
                ...
                if (MILESTONE_TYPES.has(event.eventType) && event.mediaId) {
                    await emitMilestoneIfNew(request.profileId, event.mediaId, event.sessionId ?? null, event.eventType, event.mediaType ?? 'UNKNOWN', event.positionMs ?? undefined);
                    continue;
                }
