...
function deriveEnrichmentStatus(row) {
    ...
    if (hasExternalId && hasSynopsis)
        return 'matched';
    if (!hasExternalId && !hasSynopsis)
        return 'unmatched';
    return 'partial';
}
function filterString(names) {
...
function computeWatchState(profileId, progress) {
    if (!profileId)
        return null;
...
export async function catalogRoutes(app, opts = {}) {
    ...
    app.get('/series/:id', async (request, reply) => {
        ...
        if (!seriesRow)
            return reply.status(404).send({ error: 'Series not found' });
        const [genreRows, availCountRows, seasonRows, seriesVariantRows, prefs, availEpCountRows, videoRows, creditRows,] = await Promise.all([
            db
                .select({ name: genres.name })
                .from(seriesGenres)
            ...
            db
                ...
                episodeCount: sql `cast(count(${episodes.id}) as integer)`,
            })
                .from(seasons)
            ...
            db
                .select({
                seasonNumber: seasons.seasonNumber,
                cnt: sql `cast(count(distinct ${episodeAvailabilities.episodeId}) as integer)`,
            })
                .from(seasons)
                .leftJoin(episodes, eq(episodes.seasonId, seasons.id))
                .leftJoin(episodeAvailabilities, and(eq(episodeAvailabilities.episodeId, episodes.id), eq(episodeAvailabilities.status, 'AVAILABLE')))
                .where(eq(seasons.seriesId, id))
    ...
    // ---------------------------------------------------------------------------
    app.get('/series/:id/seasons/:seasonNumber/episodes', async (request, reply) => {
        ...
        const [availCountRows, epVariantRaws, progressRows] = await Promise.all([
            db
                ...
                .where(and(inArray(episodeAvailabilities.episodeId, episodeIds), eq(episodeAvailabilities.status, 'AVAILABLE')))
            ...
            profileId
                ? db
                    ...
                    .where(and(eq(viewingProgress.profileId, profileId), eq(viewingProgress.mediaType, 'EPISODE'), inArray(viewingProgress.mediaId, episodeIds)))
                : Promise.resolve([]),
        ]);
