...
export async function embeddingBackfillRoutes(app) {
    ...
    app.get('/admin/embedding-backfill/coverage', async (_request, reply) => {
        ...
        const [movieCoverage] = await db
            .select({
            withOverview: sql `count(*) filter (where synopsis is not null and synopsis != '')`,
            withKeywords: sql `count(*) filter (where keywords is not null and jsonb_array_length(keywords::jsonb) > 0)`,
            withLanguage: sql `count(*) filter (where original_language is not null)`,
        })
            .from(movies)
        ...
        const safe = (n) => (total > 0 ? Math.round((n / total) * 100) / 100 : 0);
...
//# sourceMappingURL=embedding-backfill.js.map