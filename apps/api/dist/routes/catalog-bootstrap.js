...
export async function catalogBootstrapRoutes(app, opts) {
    ...
    app.get('/catalog-bootstrap/status', async (_request, reply) => {
        const [run] = await db
            ...
            .limit(1);
        if (!run)
            return reply.status(404).send({ error: 'No bootstrap run found' });
        return run;
    });
}
//# sourceMappingURL=catalog-bootstrap.js.map