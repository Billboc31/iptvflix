import { count, countDistinct, desc, gte, inArray, sql } from 'drizzle-orm';
...
export async function adminRoutes(app) {
    ...
    app.get('/admin/interaction-stats', async (_request, reply) => {
        ...
        const [dailyRows, topTypeRows, profileCountRow, totalRow] = await Promise.all([
            db
                .select({
                day: sql `date_trunc('day', ${profileInteractionEvents.occurredAt})::date::text`,
                count: count(),
            })
    ...
    app.get('/admin/taste-stats', async (_request, reply) => {
        ...
        const oldest = tasteRows.reduce((acc, r) => (!acc || r.builtAt < acc ? r.builtAt : acc), null);
        return reply.send({
    ...
    app.get('/admin/interaction-health', async (_request, reply) => {
        const [milestoneRows, startedRows, zeroEventRows, totalProfileRow] = await Promise.all([
            db
                ...
                .from(profileInteractionEvents)
                .where(inArray(profileInteractionEvents.eventType, [
        ...
        return reply.send({
            ...
            milestoneCoveragePercent: startedSet.size > 0
