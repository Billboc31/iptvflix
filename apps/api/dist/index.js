...
import { PORT, CORS_ORIGIN, TMDB_API_KEY, JWT_SECRET, SYNC_SCHEDULER_ENABLED, SOURCE_SYNC_CADENCE_MINUTES, DISCOVERY_CADENCE_MINUTES, SOURCE_SYNC_CONCURRENCY, SCHEDULER_STARTUP_DELAY_MS, CATALOG_REFRESH_ENABLED, CATALOG_REFRESH_CADENCE_HOURS, OPENAI_API_KEY, SEGMENT_REFRESH_ENABLED, SEGMENT_REFRESH_CADENCE_HOURS, SEGMENT_REFRESH_RECENT_DAYS, INTRODB_BASE_URL, THEINTRODB_BASE_URL, } from './config/env.js';
...
await app.register(async function protectedScope(protectedApp) {
    ...
    const enrichmentService = TMDB_API_KEY
        ? new MetadataEnrichmentService(db, new TmdbClient({ apiKey: TMDB_API_KEY }), undefined, embeddingService
            ? (mediaId, mediaType) => {
    ...
    if (TMDB_API_KEY) {
        ...
        const bootstrapService = new CatalogBootstrapService(db, new TmdbClient({ apiKey: TMDB_API_KEY }), undefined, bootstrapEnrichmentService);
...
const discoveryPoolService = TMDB_API_KEY && discoveryService
    ? new DiscoveryCandidatePoolService(db, new TmdbClient({ apiKey: TMDB_API_KEY }), discoveryService)
    : null;
...
const segmentSyncService = TMDB_API_KEY
    ? new SegmentSyncService(db, new TmdbClient({ apiKey: TMDB_API_KEY }), segmentProviders, segmentProviderPriority)
    : null;
