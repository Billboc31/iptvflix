...
async function main() {
    ...
    const plannerService = new LlmQueryPlannerService(createOpenAiPlannerProvider(OPENAI_API_KEY, LLM_PLANNER_MODEL));
    ...
    for (const bench of BENCHMARKS) {
        ...
        if (!plan.plannerFallback) {
            ...
            if (plan.desiredThemes.length)
                console.log(`  themes:          ${plan.desiredThemes.join(', ')}`);
            if (plan.desiredTone.length)
                console.log(`  tone:            ${plan.desiredTone.join(', ')}`);
            if (plan.avoidSignals.length)
                console.log(`  avoid:           ${plan.avoidSignals.join(', ')}`);
            if (plan.hardFilters.maxRuntimeMinutes)
                console.log(`  maxRuntime:      ${plan.hardFilters.maxRuntimeMinutes} min`);
            if (plan.userConstraints.length)
                console.log(`  userConstraints: ${plan.userConstraints.join(', ')}`);
    ...
    const avg = (key) => scores.reduce((s, c) => s + c[key], 0) / scores.length;
    console.log('=== Summary ===');
...
//# sourceMappingURL=llm-planner-benchmarks.js.map