...
export async function shelfInstancesRoutes(app) {
    app.post('/v1/shelf-instances/generate', async (request, reply) => {
        const { profileId, ...body } = request.body ?? {};
        if (!profileId || typeof profileId !== 'string') {
            return reply.status(400).send({ error: 'profileId is required' });
        }
        try {
            const result = await generateShelfFromSeeds(profileId, body, request.log);
    ...
    app.post('/v1/shelf-instances/:id/refresh', async (request, reply) => {
