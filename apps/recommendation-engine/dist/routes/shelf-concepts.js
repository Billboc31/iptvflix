...
import { OPENAI_API_KEY, EMBEDDING_MODEL_NAME } from '../config.js';
...
export async function shelfConceptsRoutes(app) {
    app.post('/v1/shelf-concepts/generate', async (request, reply) => {
    ...
    app.post('/v1/shelf-concepts/:id/preview', async (request, reply) => {
        ...
        const plan = buildQueryPlanFromShelfConcept({
            ...concept,
            desiredMediaTypes: concept.desiredMediaTypes,
        });
        const mediaTypes = plan.mediaTypes.map((t) => t.toLowerCase());
        ...
        const finalResult = await runRecommendationFromPlan(plan, { profileId, mediaTypes: [...mediaTypes], limit: 20, debug }, String(request.id), request.log);
    ...
    app.post('/v1/shelf-concepts/:id/feedback', async (request, reply) => {
        const { id } = request.params;
