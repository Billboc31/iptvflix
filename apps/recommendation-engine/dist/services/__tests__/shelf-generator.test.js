...
describe('buildSeedQueryPlan', () => {
    it('produces a non-empty semanticIntent from 3 movie seeds', async () => {
        ...
        const { plan, inferredGenreIds, seedTitles } = await buildSeedQueryPlan(SEED_MOVIE_IDS, ['movie']);
...
describe('buildSeedQueryPlan — seed validation', () => {
    it('throws ValidationError when a requested movie seed ID is missing from the database', async () => {
        ...
        await expect(buildSeedQueryPlan(SEED_MOVIE_IDS, ['movie'])).rejects.toThrow('Seed not found: MOVIE:mov-c');
    });
});
...
describe('buildSeedQueryPlan — ambiguous intents (integration)', () => {
    it.skipIf(!canRun)('"Aventures à travers le temps" — semanticIntent is non-empty and not a lexical copy', async () => {
...
//# sourceMappingURL=shelf-generator.test.js.map