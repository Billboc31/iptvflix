...
import { shelfConcepts, shelfConceptFatigue, profileInteractionEvents, profiles, discoveryCandidate, movies, series, profileTaste, } from '../db/schema.js';
import { SHELF_CONCEPT_PERSONALIZED_RATIO, SHELF_CONCEPT_EXPLORATION_RATIO, SHELF_CONCEPT_DISCOVERY_RATIO, SHELF_CONCEPT_BATCH_SIZE, SHELF_CONCEPT_TTL_HOURS, SHELF_CONCEPT_MIN_POOL_SIZE, SHELF_CONCEPT_SEMANTIC_DEDUP_THRESHOLD, } from '../config.js';
import { buildShelfConceptPrompt } from '../prompts/shelf-concept-generator-v1.js';
const PROMPT_VERSION = 'shelf-concept-v1';
const COLD_START_THRESHOLD = 3;
const MAX_STRING_LEN = 100;
const ALLOWED_GEN_TYPES = new Set(['PERSONALIZED', 'EXPLORATION', 'DISCOVERY', 'FIXED', 'EDITORIAL']);
const ALLOWED_FRESHNESS = new Set(['AVAILABLE_NOW', 'NEW_RELEASES', 'ALL', null, undefined]);
const ALLOWED_MEDIA_TYPES = new Set(['MOVIE', 'SERIES']);
function cap(s) {
...
function normalizeRatios(p, e, d) {
    const sum = p + e + d;
    if (sum <= 0)
        return { personalized: 0.7, exploration: 0.2, discovery: 0.1 };
    return { personalized: p / sum, exploration: e / sum, discovery: d / sum };
}
async function getFatigueStates(profileId, conceptIds) {
...
function toApiModel(row) {
    return {
        ...
        desiredMediaTypes: row.desiredMediaTypes ?? [],
        freshnessPolicy: row.freshnessPolicy ?? null,
        active: row.active,
...
export class ShelfConceptGeneratorService {
    ...
    embeddingOpenai;
    embeddingModelName;
    constructor(openai, embeddingOpenai, embeddingModelName) {
        this.openai = openai;
        this.embeddingOpenai = embeddingOpenai;
        this.embeddingModelName = embeddingModelName;
    }
    async buildProfileContext(profileId) {
        ...
        const seriesPlayEvents = interactionEvents.filter((e) => e.mediaType === 'SERIES' && (e.eventType === 'PLAY_STARTED' || e.eventType === 'PLAY_COMPLETED'));
        ...
        for (let i = 0; i < seriesPlayEvents.length && !bingeTendency; i++) {
            ...
            const inWindow = seriesPlayEvents.filter((e) => e.occurredAt.getTime() >= windowStart && e.occurredAt.getTime() <= windowEnd).length;
            if (inWindow >= 3)
                bingeTendency = true;
        }
        ...
        const genreScores = (taste?.genreScores ?? {});
    ...
    validateConcept(concept, existingConcepts) {
        ...
        if (!Array.isArray(concept.desiredMediaTypes) || !concept.desiredMediaTypes.every((t) => ALLOWED_MEDIA_TYPES.has(t))) {
    ...
    async getActivePool(profileId) {
        const now = new Date();
        const rows = await db
            .select()
            .from(shelfConcepts)
            .where(and(eq(shelfConcepts.profileId, profileId), eq(shelfConcepts.active, true), or(isNull(shelfConcepts.expiresAt), gte(shelfConcepts.expiresAt, now))))
    ...
    async needsRefresh(profileId) {
        ...
        try {
            ...
            if (newest && tasteRow && tasteRow.builtAt > new Date(newest.createdAt))
                return true;
        }
        catch {
    ...
    async generateConcepts(profileId, opts) {
        if (!this.openai)
        ...
        let rawConcepts;
        if (parsed && typeof parsed === 'object' && 'concepts' in parsed && Array.isArray(parsed.concepts)) {
        ...
        if (this.embeddingOpenai && existingConcepts.length > 0) {
            const existingEmbeddings = await Promise.all(existingConcepts.map((c) => this.embeddingOpenai.embeddings.create({ model: this.embeddingModelName, input: c.semanticIntent.trim() }).then((r) => r.data[0]?.embedding ?? null).catch(() => null)));
            for (const emb of existingEmbeddings) {
                if (emb)
                    sessionEmbeddings.push(emb);
            }
        }
        for (const raw of rawConcepts) {
            ...
            const [inserted] = await db
                ...
                desiredMediaTypes: (Array.isArray(raw.desiredMediaTypes) ? raw.desiredMediaTypes.filter((t) => ALLOWED_MEDIA_TYPES.has(t)) : []),
                ...
                .returning();
            if (inserted)
                persisted.push(toApiModel(inserted));
        }
        return persisted;
    }
    async applyFeedback(conceptId, signal) {
        if (signal === 'good') {
...
//# sourceMappingURL=shelf-concept-generator.js.map