...
function mergeCandidates(textCandidates, semanticCandidates) {
    const byId = new Map();
...
async function fetchPopularityFallbackPool(mediaTypes, poolSize) {
...
export async function runRecommendationFromPlan(plan, opts, requestId, log) {
    ...
    if (!rerankerResult.available)
        fallbackFlags.push('hybrid-reranker');
