...
import { movies, series, movieGenres, seriesGenres, genres, movieAvailabilities, seriesAvailabilities, mediaCredits, viewingProgress, profileTaste, profileMediaExposure, } from '../../db/schema.js';
...
async function loadTasteSignals(profileId) {
    ...
    if (!row)
        return null;
    return {
        genreScores: (row.genreScores ?? {}),
        positiveMediaIds: new Set(row.positiveMediaIds ?? []),
        negativeMediaIds: new Set(row.negativeMediaIds ?? []),
        dislikedMediaIds: new Set(row.dislikedMediaIds ?? []),
        notInterestedMediaIds: new Set(row.notInterestedMediaIds ?? []),
...
async function loadExposureCounts(profileId, mediaIds) {
...
async function enrichCandidates(candidates, profileId, exposureCounts) {
    ...
    const [movieGenreRows, seriesGenreRows, allGenreRows, availMovieRows, availSeriesRows, movieMetaRows, seriesMetaRows, creditRows, progressRows,] = await Promise.all([
        movieIds.length > 0
    ...
    const creditPersonIdsMap = new Map();
    ...
    return candidates.map((c) => {
        const genreIds = c.mediaType === 'movie' ? (movieGenreMap.get(c.id) ?? []) : (seriesGenreMap.get(c.id) ?? []);
        ...
        return {
            ...
            productionCountries: (movieMeta?.productionCountries ?? seriesMeta?.productionCountries ?? []),
...
export function computePeopleAffinity(c, personScores) {
    if (c.creditPersonIds.length === 0)
        return 0.5;
    let bestScore = 0;
...
export function computeFranchiseAffinity(c, franchiseScores) {
    if (!c.collectionId)
        return 0.5;
...
function extractCountryCodes(productionCountries) {
    const codes = [];
...
export function computeLanguageAffinity(c, languageScores, countryScores) {
    let langPart = 0.5;
    ...
    if (codes.length > 0) {
        ...
        for (const code of codes) {
            ...
            if (s !== undefined && (bestCountry === undefined || s > bestCountry))
                bestCountry = s;
        }
...
export function computeDecadeAffinity(c, decadeScores) {
    if (c.year == null)
        return 0.5;
...
export const HARD_FILTER_UNKNOWN_POLICY = 'STRICT_EXCLUDE_UNKNOWN';
export function passesHardFilters(c, queryPlan) {
    const { hardFilters, mediaTypes } = queryPlan;
    const candidateMediaType = c.mediaType.toUpperCase();
    if (mediaTypes.length > 0 && mediaTypes.length < 2 && !mediaTypes.includes(candidateMediaType))
        return false;
    if (hardFilters.maxRuntimeMinutes != null) {
        if (c.durationMinutes == null || c.durationMinutes > hardFilters.maxRuntimeMinutes)
            return false;
    }
    if (hardFilters.minReleaseYear != null || hardFilters.maxReleaseYear != null) {
        if (c.year == null)
            return false;
        if (hardFilters.minReleaseYear != null && c.year < hardFilters.minReleaseYear)
            return false;
        if (hardFilters.maxReleaseYear != null && c.year > hardFilters.maxReleaseYear)
            return false;
    }
    ...
    if (hardFilters.audioLanguages && hardFilters.audioLanguages.length > 0) {
        if (c.originalLanguage == null || !hardFilters.audioLanguages.includes(c.originalLanguage))
            return false;
    }
    return true;
}
...
function buildReasons(semantic, genreAffinity, languageAffinity, decadeAffinity, genreNames, peopleAffinity, keywordAffinity) {
    const reasons = [];
    if (semantic > 0.7)
        reasons.push('strong semantic match');
    else if (semantic > 0.5)
        reasons.push('semantic match');
    if (genreAffinity > 0.6 && genreNames.length > 0)
        reasons.push(`strong ${genreNames[0].toLowerCase()} genre affinity`);
    else if (genreAffinity > 0.3 && genreNames.length > 0)
        reasons.push(`${genreNames[0].toLowerCase()} genre affinity`);
    if (languageAffinity > 0.7)
        reasons.push('preferred language');
    if (decadeAffinity > 0.7)
        reasons.push('preferred era');
    if (peopleAffinity > 0.7)
        reasons.push('liked cast/crew');
    if (keywordAffinity > 0.7)
        reasons.push('strong keyword match');
    if (reasons.length === 0)
        reasons.push('discovery');
    return reasons;
}
export async function runHybridReranker(ctx, candidates) {
    ...
    try {
        ...
        const [exposureCounts, taste] = await Promise.all([
            ctx.request.profileId ? loadExposureCounts(ctx.request.profileId, allIds) : Promise.resolve(new Map()),
        ...
        const output = diversified.map((c) => ({
            id: c.id,
        ...
        ctx.log.info({ requestId: ctx.requestId, stage: 'hybrid-reranker', durationMs: Date.now() - start, inputCount: candidates.length, outputCount: output.length }, 'stage complete');
        return {
            stage: 'hybrid-reranker',
            available: true,
            durationMs: Date.now() - start,
        ...
        return {
            stage: 'hybrid-reranker',
            available: false,
            reason: `reranker error: ${err.message}`,
            durationMs: Date.now() - start,
            inputCount: candidates.length,
            outputCount: 0,
            candidates: [],
...
//# sourceMappingURL=hybrid-reranker.js.map