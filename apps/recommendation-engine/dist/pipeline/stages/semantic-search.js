...
let detectedColumnType = null;
async function checkPgvector() {
    if (pgvectorAvailable !== null)
        return pgvectorAvailable;
    try {
        const [extRows, colRows] = await Promise.all([
            pgClient `SELECT COUNT(*) AS count FROM pg_extension WHERE extname = 'vector'`,
            pgClient `
...
function toDbMediaType(t) {
    return t === 'series' ? 'SERIES' : 'MOVIE';
}
function fromDbMediaType(t) {
    return t.toUpperCase() === 'SERIES' ? 'series' : 'movie';
}
export async function runSemanticSearch(ctx, inputCandidates) {
    ...
    const retrievalLimit = Math.min(ctx.candidatePoolSize ?? SEMANTIC_RETRIEVAL_LIMIT, SEMANTIC_RETRIEVAL_MAX_CAP);
    ...
    let eligibleCount = 0;
    let detectedModels = [];
    let usePgvector = null;
    let queryVectorDim = null;
    try {
        // Pre-flight: total corpus count (any model)
        const totalRows = await pgClient `SELECT COUNT(*) AS count FROM media_embeddings`;
        ...
        const eligibleRows = await pgClient `
    ...
    `;
        ...
        const detectedModelsRows = await pgClient `
      SELECT DISTINCT model_provider || '/' || model_name AS m FROM media_embeddings LIMIT 10
    `;
        detectedModels = detectedModelsRows.map((r) => r.m);
        if (eligibleCount === 0) {
            ctx.log.warn({ requestId: ctx.requestId, configuredModel: `${EMBEDDING_MODEL_PROVIDER}/${EMBEDDING_MODEL_NAME}`, detectedModels, totalEmbeddings: totalCount }, 'semantic-search: no embeddings match configured model');
            return {
                stage: 'semantic-search',
                available: false,
        ...
        const [queryVector, anchorVector] = await Promise.all([
            ...
            useAnchorBlend ? embedQuery(openai, semanticAnchor) : Promise.resolve([]),
        ]);
    ...
    `;
        ...
        return {
            stage: 'semantic-search',
            available: false,
            reason: `semantic search error: ${err.message}`,
            durationMs: Date.now() - start,
            inputCount: inputCandidates.length,
            outputCount: 0,
            diagnostics: {
