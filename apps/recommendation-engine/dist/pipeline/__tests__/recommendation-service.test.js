...
function makePlan(semanticIntent) {
    return {
        schemaVersion: '1',
        rawQuery: '',
        displayTitle: 'Test',
...
function mockSelectFrom(rows) {
    mockDb.select.mockReturnValueOnce({
        from: vi.fn().mockResolvedValue(rows),
    });
}
// Mock the popularity fallback pool (select().from().orderBy().limit())
function mockSelectFromOrderByLimit(rows) {
    mockDb.select.mockReturnValueOnce({
        from: vi.fn().mockReturnValue({
...
const MOVIE_GENRE_ROWS = [
    ...
    ...['mov-0', 'mov-1', 'mov-2', 'mov-3', 'mov-4', 'mov-5', 'mov-6'].map((id) => ({
        movieId: id,
        genreId: 'genre-drama',
    })),
];
...
describe('runRecommendationFromPlan', () => {
    it('promotes semantically relevant candidate from vector rank >3 to final top-3 via profile affinity (T124: boost bounded by semantic relevance)', async () => {
        ...
        const result = await runRecommendationFromPlan(plan, { profileId: 'profile-a', mediaTypes: ['movie'], limit: 8 }, 'test-req', makeLogger());
    ...
    it('returns SCORE_MODEL_V2 version in engineMetadata for all paths', async () => {
        ...
        const result = await runRecommendationFromPlan(plan, { profileId: 'profile-a', mediaTypes: ['movie'], limit: 8 }, 'test-req-v2', makeLogger());
        expect(result.engineMetadata.rerankerVersion).toBe('v2');
    });
    it('propagates candidatePoolSize from opts to PipelineContext', async () => {
        let capturedCtx;
        mockSemanticFn.mockImplementationOnce(async (ctx) => {
            capturedCtx = ctx;
            return {
                stage: 'semantic-search',
                available: true,
        ...
        setupRerankerMocks();
        const plan = makePlan('Suspense films');
        await runRecommendationFromPlan(plan, { profileId: 'profile-a', mediaTypes: ['movie'], limit: 5, candidatePoolSize: 50 }, 'test-pool-size', makeLogger());
    ...
    it('cold-start: empty semanticIntent falls back to popularity pool without error', async () => {
        ...
        const plan = makePlan('');
        const result = await runRecommendationFromPlan(plan, { mediaTypes: ['movie'], limit: 5 }, 'test-cold', makeLogger());
        expect(result).toBeDefined();
        expect(result.engineMetadata.rerankerVersion).toBe('v2');
        expect(result.engineMetadata.fallbackFlags).toContain('popularity-fallback');
    });
});
//# sourceMappingURL=recommendation-service.test.js.map